...
var MRS = (function(MRS, $) {
	MRS.automaticallyCheckRecipient = function() {
        var $recipientFields = $("#send_money_recipient, #transfer_asset_recipient, " +
	...
	$("#send_message_modal, #send_money_modal, #add_contact_modal, #set_account_property_modal, #delete_account_property_modal").on("show.bs.modal", function(e) {
    ...
    modal.on("click", "span.recipient_selector button, span.plain_address_selector button", function(e) {
		if (!Object.keys(MRS.contacts).length) {
			e.preventDefault();
			e.stopPropagation();
			return;
		}
		...
		for (var accountId in MRS.contacts) {
			if (!MRS.contacts.hasOwnProperty(accountId)) {
				continue;
			}
			$list.append("<li><a href='#' data-contact-id='" + accountId + "' data-contact='" + String(MRS.contacts[accountId].name).escapeHTML() + "'>" + String(MRS.contacts[accountId].name).escapeHTML() + "</a></li>");
		}
	});
	...
	modal.on("keyup blur show", ".plain_address_selector_input", function() {
		...
		var contactInfo;
		if (MRS.contacts[currentValue]) {
			contactInfo = MRS.contacts[currentValue]['name'];
		} else {
			contactInfo = " ";
		}
	...
	MRS.forms.sendMoneyComplete = function(response, data) {
		if (!(data["_extra"] && data["_extra"].convertedAccount) && !(data.recipient in MRS.contacts)) {
			$.growl($.t("success_send_money") + " <a href='#' data-account='" + MRS.getAccountFormatted(data, "recipient") + "' data-toggle='modal' data-target='#add_contact_modal' style='text-decoration:underline'>" + $.t("add_recipient_to_contacts_q") + "</a>", {
				"type": "success"
			});
		} else {
	...
	MRS.sendMoneyShowAccountInformation = function(accountId) {
		MRS.getAccountError(accountId, function(response) {
	...
	MRS.getAccountError = function(accountId, callback) {
			...
			if (response.publicKey) {
				if (response.name){
					result = {
						...
						"message": $.t("recipient_info_with_name", {
							"name" : MRS.unescapeRespStr(response.name),
							"amount": MRS.formatAmount(response.unconfirmedBalanceMQT, false, true),
				...
				if (response.errorCode) {
					if (response.errorCode == 4) {
						result = {
							"type": "danger",
							"message": $.t("recipient_malformed") + (!MRS.isRsAccount(accountId) ? " " + $.t("recipient_alias_suggestion") : ""),
							"account": null
						};
						...
						result = {
							"type": "danger",
							"message": $.t("recipient_problem") + " " + MRS.unescapeRespStr(response.errorDescription),
							"account": null
						};
					}
				} else {
	...
	MRS.checkRecipient = function(account, modal) {
		...
		if (MRS.isRsAccount(account)) {
			...
			if (address.set(account)) {
				MRS.getAccountError(account, function(response) {
					if (response.noPublicKey && account!=MRS.accountRS) {
				...
				if (address.guess.length == 1) {
					callout.removeClass(classes).addClass("callout-danger").html($.t("recipient_malformed_suggestion", {
						"recipient": "<span class='malformed_address' data-address='" + MRS.escapeRespStr(address.guess[0]) + "' onclick='MRS.correctAddressMistake(this);'>" + address.format_guess(address.guess[0], account) + "</span>"
					})).show();
					...
					for (var i = 0; i < address.guess.length; i++) {
						html += "<li><span class='malformed_address' data-address='" + MRS.escapeRespStr(address.guess[i]) + "' onclick='MRS.correctAddressMistake(this);'>" + address.format_guess(address.guess[i], account) + "</span></li>";
					}
		...
		} else if (!MRS.isNumericAccount(account)) {
			if (account.charAt(0) != '@') {
				MRS.storageSelect("contacts", [{
					...
					if (!error && contact.length) {
						contact = contact[0];
						MRS.getAccountError(contact.accountRS, function(response) {
							if (response.noPublicKey && account!=MRS.account) {
								modal.find(".recipient_public_key").show();
							} else {
								modal.find("input[name=recipientPublicKey]").val("");
								modal.find(".recipient_public_key").hide();
							...
							callout.removeClass(classes).addClass("callout-" + response.type).html($.t("contact_account_link", {
								"account_id": MRS.getAccountFormatted(contact, "account")
						...
						MRS.checkRecipientAlias(account, modal);
					} else {
						callout.removeClass(classes).addClass("callout-danger").html($.t("recipient_malformed")).show();
					}
				});
	...
	MRS.checkRecipientAlias = function(account, modal) {
		var classes = "callout-info callout-danger callout-warning";
		var callout = modal.find(".account_info").first();
		var accountInputField = modal.find("input[name=converted_account_id]");
			...
			if (response.errorCode) {
				...
				if (response.aliasURI) {
					...
					if (match && match[1]) {
						...
						if (MRS.isNumericAccount(match[1])) {
							var address = new MetroAddress();
						...
						MRS.getAccountError(match[1], function(response) {
							...
							callout.removeClass(classes).addClass("callout-" + response.type).html($.t("alias_account_link", {
								"account_id": MRS.escapeRespStr(match[1])
							}) + " " + response.message + " " + $.t("alias_last_adjusted", {
								"timestamp": MRS.formatTimestamp(timestamp)
							})).show();

							if (response.type == "info" || response.type == "warning") {
								accountInputField.val(MRS.escapeRespStr(match[1]));
							}
						});
					} else {
						callout.removeClass(classes).addClass("callout-danger").html($.t("alias_account_no_link") + (!alias ? $.t("error_uri_empty") : $.t("uri_is", {
							"uri": MRS.escapeRespStr(alias)
						}))).show();
					}
				} else if (response.aliasName) {
					callout.removeClass(classes).addClass("callout-danger").html($.t("error_alias_empty_uri")).show();
				} else {
					callout.removeClass(classes).addClass("callout-danger").html(response.errorDescription ? $.t("error") + ": " + MRS.escapeRespStr(response.errorDescription) : $.t("error_alias")).show();
				}
			}
		});
	};
