...
public final class PopOff extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        try {
            Metro.getBlockchainProcessor().setGetMoreBlocks(false);
            if (numBlocks > 0) {
                blocks = Metro.getBlockchainProcessor().popOffTo(Metro.getBlockchain().getHeight() - numBlocks);
            } else if (height > 0) {
                blocks = Metro.getBlockchainProcessor().popOffTo(height);
            } else {
                return JSONResponses.missing("numBlocks", "height");
            }
        } finally {
            Metro.getBlockchainProcessor().setGetMoreBlocks(true);
        }
        ...
        if (keepTransactions) {
            blocks.forEach(block -> Metro.getTransactionProcessor().processLater(block.getTransactions()));
        }
        return response;
    }
