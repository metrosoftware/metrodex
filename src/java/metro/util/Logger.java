...
/**
 * Handle logging for the Metro node server
 */
public final class Logger {
     ...
     * The metro/conf/logging-default.properties and metro/conf/logging.properties configuration
    ...
    static {
        ...
        System.setProperty("java.util.logging.manager", "metro.util.MetroLogManager");
        if (!(LogManager.getLogManager() instanceof MetroLogManager)) {
            System.setProperty("java.util.logging.manager",
                    (oldManager != null ? oldManager : "java.util.logging.LogManager"));
        }
        if (! Boolean.getBoolean("metro.doNotConfigureLogging")) {
            try {
                Properties loggingProperties = new Properties();
                Metro.loadProperties(loggingProperties, "logging-default.properties", true);
                Metro.loadProperties(loggingProperties, "logging.properties", false);
                Metro.updateLogFileHandler(loggingProperties);
        ...
        log = org.slf4j.LoggerFactory.getLogger(Metro.class);
        enableStackTraces = Metro.getBooleanProperty("metro.enableStackTraces");
        enableLogTraceback = Metro.getBooleanProperty("metro.enableLogTraceback");
    ...
    public static void shutdown() {
        if (LogManager.getLogManager() instanceof MetroLogManager) {
            ((MetroLogManager) LogManager.getLogManager()).mtrShutdown();
        }
    }
