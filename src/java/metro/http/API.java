...
import static metro.http.JSONResponses.INCORRECT_ADMIN_PASSWORD;
import static metro.http.JSONResponses.LOCKED_ADMIN_PASSWORD;
import static metro.http.JSONResponses.MISSING_ADMIN_PASSWORD;
import static metro.http.JSONResponses.NO_PASSWORD_IN_CONFIG;
...
public final class API {
    ...
    public static final String adminPassword = Metro.getStringProperty("metro.adminPassword", "", true);
    static final boolean disableAdminPassword;
    static final int maxRecords = Metro.getIntProperty("metro.maxAPIRecords");
    static final boolean enableAPIUPnP = Metro.getBooleanProperty("metro.enableAPIUPnP");
    public static final int apiServerIdleTimeout = Metro.getIntProperty("metro.apiServerIdleTimeout");
    public static final boolean apiServerCORS = Metro.getBooleanProperty("metro.apiServerCORS");
    private static final String forwardedForHeader = Metro.getStringProperty("metro.forwardedForHeader");
    ...
    static {
        List<String> disabled = new ArrayList<>(Metro.getStringListProperty("metro.disabledAPIs"));
        Collections.sort(disabled);
        disabledAPIs = Collections.unmodifiableList(disabled);
        disabled = Metro.getStringListProperty("metro.disabledAPITags");
        Collections.sort(disabled);
        ...
        List<String> allowedBotHostsList = Metro.getStringListProperty("metro.allowedBotHosts");
        ...
        boolean enableAPIServer = Metro.getBooleanProperty("metro.enableAPIServer");
        if (enableAPIServer) {
            final int port = Constants.isTestnet ? TESTNET_API_PORT : Metro.getIntProperty("metro.apiServerPort");
            final int sslPort = Constants.isTestnet ? TESTNET_API_SSLPORT : Metro.getIntProperty("metro.apiServerSSLPort");
            final String host = Metro.getStringProperty("metro.apiServerHost");
            disableAdminPassword = Metro.getBooleanProperty("metro.disableAdminPassword") || ("127.0.0.1".equals(host) && adminPassword.isEmpty());

            apiServer = new Server();
            ServerConnector connector;
            boolean enableSSL = Metro.getBooleanProperty("metro.apiSSL");
            //
            ...
            if (enableSSL) {
                ...
                String keyStorePath = Paths.get(Metro.getUserHomeDir()).resolve(Paths.get(Metro.getStringProperty("metro.keyStorePath"))).toString();
                Logger.logInfoMessage("Using keystore: " + keyStorePath);
                sslContextFactory.setKeyStorePath(keyStorePath);
                sslContextFactory.setKeyStorePassword(Metro.getStringProperty("metro.keyStorePassword", null, true));
                ...
                sslContextFactory.setKeyStoreType(Metro.getStringProperty("metro.keyStoreType"));
                List<String> ciphers = Metro.getStringListProperty("metro.apiSSLCiphers");
                if (!ciphers.isEmpty()) {
            ...
            String apiResourceBase = Metro.getStringProperty("metro.apiResourceBase");
            if (apiResourceBase != null) {
                ...
                apiHandler.setWelcomeFiles(new String[]{Metro.getStringProperty("metro.apiWelcomeFile")});
            }

            String javadocResourceBase = Metro.getStringProperty("metro.javadocResourceBase");
            ...
            ServletHolder servletHolder = apiHandler.addServlet(APIServlet.class, "/metro");

            servletHolder.getRegistration().setMultipartConfig(new MultipartConfigElement(
                    null, Math.max(Metro.getIntProperty("metro.maxUploadFileSize"), Constants.MAX_MESSAGE_DATA_LENGTH), -1L, 0));

            servletHolder = apiHandler.addServlet(APIProxyServlet.class, "/metro-proxy");
            ...
            if (!Metro.getBooleanProperty("metro.enableAPIServerGZIPFilter", isOpenAPI)) {
                gzipHandler.setExcludedPaths("/metro", "/metro-proxy");
            }
            gzipHandler.setIncludedMethods("GET", "POST");
            gzipHandler.setMinGzipSize(metro.peer.Peers.MIN_COMPRESS_SIZE);
            ...
            if (Metro.getBooleanProperty("metro.apiFrameOptionsSameOrigin")) {
