...
public final class VoteWeighting {
    ...
    public enum VotingModel {
        ...
        MQT(1) {
            @Override
            public final long calcWeight(VoteWeighting voteWeighting, long voterId, int height) {
                long mqtBalance = Account.getAccount(voterId, height).getBalanceMQT();
                return mqtBalance >= voteWeighting.minBalance ? mqtBalance : 0;
            }
            @Override
            public final MinBalanceModel getMinBalanceModel() {
                return MinBalanceModel.MQT;
            }
        },
        ASSET(2) {
            @Override
    ...
    public enum MinBalanceModel {
        ...
        MQT(1) {
            @Override
            public final long getBalance(VoteWeighting voteWeighting, long voterId, int height) {
                return Account.getAccount(voterId, height).getBalanceMQT();
            }
        },
        ASSET(2) {
            @Override
    ...
    public void validate() throws MetroException.ValidationException {
        if (votingModel == null) {
            throw new MetroException.NotValidException("Invalid voting model");
        }
        if (minBalanceModel == null) {
            throw new MetroException.NotValidException("Invalid min balance model");
        }
        if ((votingModel == VotingModel.ASSET) && holdingId == 0) {
            throw new MetroException.NotValidException("No holdingId provided");
        }
        if (votingModel == VotingModel.ASSET && Asset.getAsset(holdingId) == null) {
            throw new MetroException.NotCurrentlyValidException("Asset " + Long.toUnsignedString(holdingId) + " not found");
        }
        if (minBalance < 0) {
            throw new MetroException.NotValidException("Invalid minBalance " + minBalance);
        }
        if (minBalance > 0) {
            if (minBalanceModel == MinBalanceModel.NONE) {
                throw new MetroException.NotValidException("Invalid min balance model " + minBalanceModel);
            }
            if (votingModel.getMinBalanceModel() != MinBalanceModel.NONE && votingModel.getMinBalanceModel() != minBalanceModel) {
                throw new MetroException.NotValidException("Invalid min balance model: " + minBalanceModel + " for voting model " + votingModel);
            }
            ...
            if (minBalanceModel == MinBalanceModel.ASSET && Asset.getAsset(holdingId) == null) {
                throw new MetroException.NotCurrentlyValidException("Invalid min balance asset: " + Long.toUnsignedString(holdingId));
            }
        }
        if (minBalance == 0 && votingModel == VotingModel.ACCOUNT && holdingId != 0) {
            throw new MetroException.NotValidException("HoldingId cannot be used in by account voting with no min balance");
        }
        if ((votingModel == VotingModel.MQT || minBalanceModel == MinBalanceModel.MQT) && holdingId != 0) {
            throw new MetroException.NotValidException("HoldingId cannot be used in by balance voting or with min balance in MQT");
        }
        if ((!votingModel.acceptsVotes() || votingModel == VotingModel.HASH) && (holdingId != 0 || minBalance != 0 || minBalanceModel != MinBalanceModel.NONE)) {
            throw new MetroException.NotValidException("With VotingModel " + votingModel + " no holdingId, minBalance, or minBalanceModel should be specified");
        }
    }
