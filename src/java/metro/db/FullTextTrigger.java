 ...
 *   CREATE ALIAS FTL_CREATE_INDEX FOR "metro.db.FullTextTrigger.createIndex"
 *       CALL FTL_CREATE(schema, table, columnList)
 *   CREATE ALIAS FTL_DROP_INDEX FOR "metro.db.FullTextTrigger.dropIndex"
 *       CALL FTL_DROP(schema, table)
 *   CREATE ALIAS FTL_SEARCH FOR "metro.db.FullTextTrigger.search"
 *       CALL FTL_SEARCH(schema, table, query, limit, offset)
 *
 * FTL_CREATE_INDEX is called to create a fulltext index for a table.  It is
 * provided as a convenience for use in MetroDbVersion when creating a new index
 * after the database has been created.
 *
 * FTL_DROP_INDEX is called to drop a fulltext index for a table.  It is
 * provided as a convenience for use in MetroDbVersion when dropping a table
 * after the database has been created.
 *
 ...
 *   COLUMNS - the primary key columns (String[]) - this is always DB_ID for MRS
 ...
 *   CREATE TRIGGER trigger_name AFTER INSERT,UPDATE,DELETE ON table_name FOR EACH ROW CALL "metro.db.FullTextTrigger"
 */
public class FullTextTrigger implements Trigger, TransactionalDb.TransactionCallback {
    ...
    /**
     * This method is called by MRS initialization to indicate MRS is active.
     *
     * This is required since database triggers will be initialized when the database
     * is opened outside the MRS environment (for example, from the H2 console)
     *
     ...
     *
     * This method should be called from MetroDbVersion when performing the database version update
     * that enables MRS fulltext search support
     */
    public static void init() {
        ...
        try (Connection conn = Db.db.getConnection();
            ...
            if (triggersExist && alreadyInitialized) {
                Logger.logInfoMessage("MRS fulltext support is already initialized");
                return;
            }
            //
            ...
            Logger.logInfoMessage("MRS fulltext schema created");
            //
            ...
            Logger.logInfoMessage("MRS fulltext aliases created");
        } catch (SQLException exc) {
            Logger.logErrorMessage("Unable to initialize MRS fulltext search support", exc);
    ...
    public static void createIndex(Connection conn, String schema, String table, String columnList)
        ...
        if (trigger == null) {
            Logger.logErrorMessage("MRS fulltext trigger for table " + tableName + " was not initialized");
        } else {
            try {
                trigger.reindexTable(conn);
    ...
    public void init(Connection conn, String schema, String trigger, String table, boolean before, int type)
                                    throws SQLException {
        //
        // Ignore the trigger if MRS is not active or this is a temporary table copy
        //
        ...
        try (Statement stmt = conn.createStatement()) {
            ...
            //
            // MRS tables use DB_ID as the primary index
            //
    ...
    private static void getIndexAccess(Connection conn) throws SQLException {
        if (!isActive) {
            throw new SQLException("MRS is no longer active");
        }
