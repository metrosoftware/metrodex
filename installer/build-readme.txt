Building the Metro installer

Note: all pathes are relative to the git repository folder (metro-private)
...
Package metro.jar using jar.sh
mkdir jre
...
Test X11 installer: java -jar metro-client-<version>.jar
Test console installer: java -jar metro-client-<version>.jar -console
...
Copy the content of Java JRE folder (C:\Program Files\Java\jre<version>) into the jre folder under your metro-private git repository preserving the folder structure
...
Test the installer using Java: java -jar metro-client-<version>.jar
Test the windows executable: metro-client-<version>.exe
...
metro.jar is now self executing, the full classpath is embedded into the jar file manifest automatically by jar.sh
