...
var MRS = (function (MRS, $, undefined) {
    ...
    MRS.resetAssetExchangeState = function () {
        assets = [];
        assetIds = [];
        closedGroups = [];
        assetSearch = false;
    ...
    MRS.setClosedGroups = function(groups) {
        closedGroups = groups;
    };

    MRS.getCurrentAsset = function() {
        return currentAsset;
    };

    function loadAssetFromURL() {
        var page = MRS.getUrlParameter("page");
        var asset = MRS.getUrlParameter("asset");
        ...
        MRS.sendRequest("getAsset", {
            ...
            if (response.errorCode) {
                ...
                MRS.loadAsset(response, false);
            }
        });
    }

    MRS.pages.asset_exchange = function (callback) {
        $(".content.content-stretch:visible").width($(".page:visible").width());
        assets = [];
        assetIds = [];
        MRS.storageSelect("assets", null, function (error, assets) {
            //select already bookmarked assets
            $.each(assets, function (index, asset) {
                MRS.cacheAsset(asset);
            });

            //check owned assets, see if any are not yet in bookmarked assets
            if (MRS.accountInfo.unconfirmedAssetBalances) {
                var newAssetIds = [];

                $.each(MRS.accountInfo.unconfirmedAssetBalances, function (key, assetBalance) {
                ...
                if (newAssetIds.length) {
                    ...
                    MRS.sendRequest("getAssets+", {
                        ...
                        if (response.assets && response.assets.length) {
                            MRS.saveAssetBookmarks(response.assets, function () {
                                MRS.loadAssetExchangeSidebar(callback);
                            });
                        } else {
                            MRS.loadAssetExchangeSidebar(callback);
                        }
    ...
    MRS.forms.addAssetBookmark = function ($modal) {
        ...
        if (MRS.isRsAccount(data.id)) {
            MRS.sendRequest("getAssetsByIssuer", {
                "account": data.id
            }, function (response) {
                if (response.errorCode) {
                    MRS.showModalError(MRS.translateServerError(response), $modal);
                } else {
                    if (response.assets && response.assets[0] && response.assets[0].length) {
                        MRS.saveAssetBookmarks(response.assets[0], MRS.forms.addAssetBookmarkComplete);
                    } else {
                        MRS.showModalError($.t("account_no_assets"), $modal);
                    }
                    //MRS.saveAssetIssuer(data.id);
                }
            });
        } else {
            MRS.sendRequest("getAsset", {
                    ...
                    MRS.sendRequest("getAssetsByIssuer", {
                        ...
                        if (response.errorCode) {
                            ...
                            if (response.assets && response.assets[0] && response.assets[0].length) {
                                ...
                                MRS.showModalError($.t("no_asset_found"), $modal);
                            }
                        }
                    });
                } else {
                    MRS.saveAssetBookmarks(new Array(response), MRS.forms.addAssetBookmarkComplete);
                }
            });
        }
    };
    ...
    $("#asset_exchange_bookmark_this_asset").on("click", function () {
        if (viewingAsset) {
            MRS.saveAssetBookmarks(new Array(viewingAsset), function (newAssets) {
                viewingAsset = false;
                MRS.loadAssetExchangeSidebar(function () {
    ...
    MRS.forms.addAssetBookmarkComplete = function (newAssets, submittedAssets) {
        assetSearch = false;
    ...
    MRS.saveAssetBookmarks = function (assetsNew, callback) {
        var newAssetIds = [];
        var newAssets = [];
        ...
        MRS.storageSelect("assets", newAssetIds, function (error, existingAssets) {
            var existingIds = [];
            ...
            if (newAssets.length == 0) {
                ...
                MRS.storageInsert("assets", "asset", newAssets, function () {
    ...
    MRS.positionAssetSidebar = function () {
        var assetExchangeSidebar = $("#asset_exchange_sidebar");
    ...
    MRS.loadAssetExchangeSidebar = function (callback) {
        ...
        for (var i = 0; i < assets.length; i++) {
            ...
            if (asset.groupName.toLowerCase() != lastGroup) {
                ...
                if (asset.groupName) {
                    ...
                    rows += " data-groupname='" + MRS.escapeRespStr(asset.groupName) + "' data-closed='" + isClosedGroup + "'>";
                    rows += "<h4 class='list-group-item-heading'>" + MRS.unescapeRespStr(asset.groupName).toUpperCase().escapeHTML() + "</h4>";
            ...
            if (MRS.accountInfo.assetBalances) {
                $.each(MRS.accountInfo.assetBalances, function (key, assetBalance) {
            ...
            rows += "data-cache='" + i + "' ";
            rows += "data-asset='" + MRS.escapeRespStr(asset.asset) + "'" + (!ungrouped ? " data-groupname='" + MRS.escapeRespStr(asset.groupName) + "'" : "");
            rows += (isClosedGroup ? " style='display:none'" : "") + " data-closed='" + isClosedGroup + "'>";
            rows += "<h4 class='list-group-item-heading'>" + MRS.escapeRespStr(asset.name) + "</h4>";
            rows += "<p class='list-group-item-text'><span>" + $.t('quantity') + "</span>: " + MRS.formatQuantity(ownsQuantityQNT, asset.decimals) + "</p>";
            rows += "</a>";
        }
        ...
        if (MRS.getUrlParameter("page") && MRS.getUrlParameter("page") == "asset_exchange" && MRS.getUrlParameter("asset")) {
    ...
    MRS.incoming.asset_exchange = function () {
        var assetExchangeSidebar = $("#asset_exchange_sidebar");
        if (!viewingAsset) {
            ...
            MRS.loadAsset(viewingAsset, true);
        }
    ...
    $("#asset_exchange_sidebar").on("click", "a", function (e, data) {
        ...
        MRS.storageSelect("assets", [{
            "asset": currentAssetID
        }], function (error, asset) {
            if (asset && asset.length && asset[0].asset == currentAssetID) {
                MRS.loadAsset(asset[0], refresh);
            }
        });
    });

    MRS.loadAsset = function (asset, refresh) {
        var assetId = asset.asset;
        currentAsset = asset;
        MRS.currentSubPage = assetId;
        ...
        if (!refresh) {
            ...
            $("#asset_account").html(MRS.getAccountLink(asset, "account"));
            $("#asset_id").html(MRS.getTransactionLink(assetId));
            $("#asset_decimals").html(MRS.escapeRespStr(asset.decimals));
            $("#asset_name").html(MRS.escapeRespStr(asset.name));
            ...
            $("#sell_asset_for_mtr").html($.t("sell_asset_for_mtr", {
                "assetName": MRS.escapeRespStr(asset.name), "symbol": MRS.constants.COIN_SYMBOL
            }));
            $("#buy_asset_with_mtr").html($.t("buy_asset_with_mtr", {
                "assetName": MRS.escapeRespStr(asset.name), "symbol": MRS.constants.COIN_SYMBOL
            }));
            ...
            MRS.sendRequest("getAsset", {
                ...
                if (!response.errorCode) {
                    if (response.asset != asset.asset || response.account != asset.account || response.accountRS != asset.accountRS || response.decimals != asset.decimals || response.description != asset.description || response.name != asset.name) {
                        MRS.storageDelete("assets", [{
                            "asset": asset.asset
                        }], function () {
                            setTimeout(function () {
                                MRS.loadPage("asset_exchange");
                    ...
                    $("#asset_quantity").html(MRS.formatQuantity(response.quantityQNT, response.decimals));
                }
            });
        ...
        if (asset.accountRS == MRS.accountRS) {
            $("#dividend_payment_link").show();
        } else {
            $("#dividend_payment_link").hide();
        }

        if (MRS.accountInfo.unconfirmedBalanceMQT == "0") {
            $("#your_mtr_balance").html("0");
            $("#buy_automatic_price").addClass("zero").removeClass("nonzero");
        } else {
            $("#your_mtr_balance").html(MRS.formatAmount(MRS.accountInfo.unconfirmedBalanceMQT));
            $("#buy_automatic_price").addClass("nonzero").removeClass("zero");
        }

        if (MRS.accountInfo.unconfirmedAssetBalances) {
            for (var i = 0; i < MRS.accountInfo.unconfirmedAssetBalances.length; i++) {
                var balance = MRS.accountInfo.unconfirmedAssetBalances[i];
                if (balance.asset == assetId) {
                    currentAsset.yourBalanceQNT = balance.unconfirmedBalanceQNT;
                    $("#your_asset_balance").html(MRS.formatQuantity(balance.unconfirmedBalanceQNT, currentAsset.decimals));
        ...
        MRS.loadAssetOrders("ask", assetId, refresh);
        MRS.loadAssetOrders("bid", assetId, refresh);
        MRS.getAssetTradeHistory(assetId, refresh);
        MRS.getAssetDividendHistory(assetId, "asset_dividend");
    };
    ...
    function processOrders(orders, type, refresh) {
        if (orders.length) {
            ...
            var quantityDecimals = MRS.getNumberOfDecimals(orders, "quantityQNT", function(val) {
                return MRS.formatQuantity(val.quantityQNT, currentAsset.decimals);
            });
            var priceDecimals = MRS.getNumberOfDecimals(orders, "priceMQT", function(val) {
                return MRS.formatOrderPricePerWholeQNT(val.priceMQT, currentAsset.decimals);
            });
            var amountDecimals = MRS.getNumberOfDecimals(orders, "totalMQT", function(val) {
                return MRS.formatAmount(MRS.calculateOrderTotalMQT(val.quantityQNT, val.priceMQT));
            });
            for (var i = 0; i < orders.length; i++) {
                order = orders[i];
                order.priceMQT = new BigInteger(order.priceMQT);
                order.quantityQNT = new BigInteger(order.quantityQNT);
                order.totalMQT = new BigInteger(MRS.calculateOrderTotalMQT(order.quantityQNT, order.priceMQT));
                sum = sum.add(order.totalMQT);
                if (i == 0 && !refresh) {
                    $("#" + (type == "ask" ? "buy" : "sell") + "_asset_price").val(MRS.calculateOrderPricePerWholeQNT(order.priceMQT, currentAsset.decimals));
                }
                var statusIcon = MRS.getTransactionStatusIcon(order);
                var className = (order.account == MRS.account ? "your-order" : "");
                rows += "<tr class='" + className + "' data-transaction='" + MRS.escapeRespStr(order.order) + "' data-quantity='" + order.quantityQNT.toString().escapeHTML() + "' data-price='" + order.priceMQT.toString().escapeHTML() + "'>" +
                    "<td>" + MRS.getTransactionLink(order.order, statusIcon, true) + "</td>" +
                    "<td>" + MRS.getAccountLink(order, "account", currentAsset.accountRS, "asset_issuer") + "</td>" +
                    "<td class='numeric'>" + MRS.formatQuantity(order.quantityQNT, currentAsset.decimals, false, quantityDecimals) + "</td>" +
                    "<td class='numeric'>" + MRS.formatOrderPricePerWholeQNT(order.priceMQT, currentAsset.decimals, priceDecimals) + "</td>" +
                    "<td class='numeric'>" + MRS.formatAmount(order.totalMQT, false, false, amountDecimals) + "</td>" +
                    "<td class='numeric'>" + MRS.formatAmount(sum, false, false, amountDecimals) + "</td>" +
                "</tr>";
            }
        ...
        MRS.dataLoadFinished($("#asset_exchange_" + type + "_orders_table"), !refresh);
    }

    MRS.loadAssetOrders = function (type, assetId, refresh) {
        ...
        async.parallel([
            function(callback) {
                ...
                MRS.sendRequest("get" + type.capitalize() + "Orders+" + assetId, params, function (response) {
            ...
            function(callback) {
                MRS.sendRequest("getExpected" + type.capitalize() + "Orders+" + assetId, params, function (response) {
                    var orders = response[type + "Orders"];
                    if (!orders) {
                        orders = [];
                    }
        ...
        function(err, results) {
            ...
            orders.sort(function (a, b) {
                if (type == "ask") {
                    return a.priceMQT - b.priceMQT;
                } else {
                    return b.priceMQT - a.priceMQT;
                }
            });
    ...
    MRS.getAssetDividendHistory = function (assetId, table) {
        ...
        MRS.sendRequest("getAssetDividends+", options, function (response) {
            var dividends = response.dividends;
            var amountDecimals = MRS.getNumberOfDecimals(dividends, "totalDividend", function(val) {
                return MRS.formatAmount(val.totalDividend);
            });
            var accountsDecimals = MRS.getNumberOfDecimals(dividends, "numberOfAccounts", function(val) {
                return MRS.formatAmount(val.numberOfAccounts);
            });
            var amountMQTPerQNTDecimals = MRS.getNumberOfDecimals(dividends, "amountMQTPerQNT", function(val) {
                return MRS.formatOrderPricePerWholeQNT(val.amountMQTPerQNT, currentAsset.decimals);
            });
            for (var i = 0; i < dividends.length; i++) {
                var dividend = dividends[i];
                dividend.numberOfAccounts = new BigInteger(dividend.numberOfAccounts.toString());
                dividend.amountMQTPerQNT = new BigInteger(dividend.amountMQTPerQNT);
                dividend.totalDividend = new BigInteger(dividend.totalDividend);
                view.data.push({
                    "timestamp": MRS.getTransactionLink(dividend.assetDividend, MRS.formatTimestamp(dividend.timestamp)),
                    "dividend_height": String(dividend.dividendHeight).escapeHTML(),
                    "total": MRS.formatAmount(dividend.totalDividend, false, false, amountDecimals),
                    "accounts": MRS.formatQuantity(dividend.numberOfAccounts, false, false, accountsDecimals),
                    "amount_per_share": MRS.formatOrderPricePerWholeQNT(dividend.amountMQTPerQNT, currentAsset.decimals, amountMQTPerQNTDecimals)
                })
            }
            view.render({
                isLoading: false,
    ...
    MRS.getAssetTradeHistory = function (assetId, refresh) {
        var options = {
            "asset": assetId,
            "firstIndex": 0,
            "lastIndex": 50
        ...
        if (assetTradeHistoryType == "you") {
            options["account"] = MRS.accountRS;
        }

        MRS.sendRequest("getTrades+" + assetId, options, function (response) {
            var exchangeTradeHistoryTable = $("#asset_exchange_trade_history_table");
            if (response.trades && response.trades.length) {
                var trades = response.trades;
                var rows = "";
                var quantityDecimals = MRS.getNumberOfDecimals(trades, "quantityQNT", function(val) {
                    return MRS.formatQuantity(val.quantityQNT, currentAsset.decimals);
                });
                var priceDecimals = MRS.getNumberOfDecimals(trades, "priceMQT", function(val) {
                    return MRS.formatOrderPricePerWholeQNT(val.priceMQT, currentAsset.decimals);
                });
                var amountDecimals = MRS.getNumberOfDecimals(trades, "sum", function(val) {
                    return MRS.formatAmount(MRS.calculateOrderTotalMQT(val.quantityQNT, val.priceMQT));
                });
                for (var i = 0; i < trades.length; i++) {
                    var trade = trades[i];
                    trade.priceMQT = new BigInteger(trade.priceMQT);
                    trade.quantityQNT = new BigInteger(trade.quantityQNT);
                    trade.totalMQT = new BigInteger(MRS.calculateOrderTotalMQT(trade.priceMQT, trade.quantityQNT));
                    rows += "<tr>" +
                        "<td>" + MRS.getTransactionLink(trade.bidOrder, MRS.formatTimestamp(trade.timestamp)) + "</td>" +
                        "<td>" + $.t(trade.tradeType) + "</td>" +
                        "<td class='numeric'>" + MRS.formatQuantity(trade.quantityQNT, currentAsset.decimals, false, quantityDecimals) + "</td>" +
                        "<td class='asset_price numeric'>" + MRS.formatOrderPricePerWholeQNT(trade.priceMQT, currentAsset.decimals, priceDecimals) + "</td>" +
                        "<td style='text-align:right;color:";
                        if (trade.buyer == MRS.account && trade.buyer != trade.seller) {
                            rows += "red";
                        } else if (trade.seller == MRS.account && trade.buyer != trade.seller) {
                            rows += "green";
                        } else {
                            rows += "black";
                        }
                    rows += "'>" + MRS.formatAmount(trade.totalMQT, false, false, amountDecimals) + "</td>" +
                        "<td>" + MRS.getAccountLink(trade, "buyer", currentAsset.accountRS, "asset_issuer") + "</td>" +
                        "<td>" + MRS.getAccountLink(trade, "seller", currentAsset.accountRS, "asset_issuer") + "</td>" +
                    "</tr>";
                }
                exchangeTradeHistoryTable.find("tbody").empty().append(rows);
                MRS.dataLoadFinished(exchangeTradeHistoryTable, !refresh);
            } else {
    ...
    $("#asset_exchange_trade_history_type").find(".btn").click(function (e) {
        ...
        MRS.getAssetTradeHistory(currentAsset.asset, true);
    });
    ...
    $("#asset_exchange_bid_orders_table tbody, #asset_exchange_ask_orders_table tbody").on("click", "td", function (e) {
        ...
        try {
            var priceMQT = new BigInteger(String($tr.data("price")));
            var quantityQNT = new BigInteger(String($tr.data("quantity")));
            var totalMQT = new BigInteger(MRS.calculateOrderTotalMQT(quantityQNT, priceMQT));

            $("#" + type + "_asset_price").val(MRS.calculateOrderPricePerWholeQNT(priceMQT, currentAsset.decimals));
            $("#" + type + "_asset_quantity").val(MRS.convertToQNTf(quantityQNT, currentAsset.decimals));
            $("#" + type + "_asset_total").val(MRS.convertToMTR(totalMQT));
        } catch (err) {
            return;
        }
        ...
        if (type == "buy") {
            try {
                var balanceMQT = new BigInteger(MRS.accountInfo.unconfirmedBalanceMQT);
            } catch (err) {
                return;
            }

            if (totalMQT.compareTo(balanceMQT) > 0) {
    ...
    $("#sell_automatic_price, #buy_automatic_price").on("click", function () {
        try {
            ...
            var price = new Big(MRS.convertToMQT(String(assetPrice.val())));
            var balanceMQT = new Big(MRS.accountInfo.unconfirmedBalanceMQT);
            var maxQuantity = new Big(MRS.convertToQNTf(currentAsset.quantityQNT, currentAsset.decimals));
            if (balanceMQT.cmp(new Big("0")) <= 0) {
                return;
            }
            ...
            if (price.cmp(new Big("0")) <= 0) {
                ...
                assetPrice.val(MRS.convertToMTR(price.toString()));
            }

            var quantity;
            if (type == "sell") {
                quantity = new Big(currentAsset.yourBalanceQNT ? MRS.convertToQNTf(currentAsset.yourBalanceQNT, currentAsset.decimals) : "0");
            } else {
                quantity = new Big(MRS.amountToPrecision(balanceMQT.div(price).toString(), currentAsset.decimals));
            }
            ...
            assetTotal.val(MRS.convertToMTR(total.toString()));
            assetTotal.css({
                "background": "",
                "color": ""
            });
        } catch (err) {
            MRS.logConsole(err.message);
        }
    });
    ...
    $("#buy_asset_quantity, #buy_asset_price, #sell_asset_quantity, #sell_asset_price").keydown(function (e) {
        ...
        MRS.validateDecimals(maxFractionLength, charCode, $(this).val(), e);
    });
    ...
    $("#sell_asset_quantity, #sell_asset_price, #buy_asset_quantity, #buy_asset_price").keyup(function () {
        ...
        try {
            var quantityQNT = new BigInteger(MRS.convertToQNT(String($("#" + orderType + "_asset_quantity").val()), currentAsset.decimals));
            var priceMQT = new BigInteger(MRS.calculatePricePerWholeQNT(MRS.convertToMQT(String($("#" + orderType + "_asset_price").val())), currentAsset.decimals));

            if (priceMQT.toString() == "0" || quantityQNT.toString() == "0") {
                $("#" + orderType + "_asset_total").val("0");
            } else {
                var total = MRS.calculateOrderTotal(quantityQNT, priceMQT, currentAsset.decimals);
    ...
    $("#asset_order_modal").on("show.bs.modal", function (e) {
        ...
        try {
            ...
            var quantityQNT = new BigInteger(MRS.convertToQNT(quantity, currentAsset.decimals));
            var priceMQT = new BigInteger(MRS.calculatePricePerWholeQNT(MRS.convertToMQT(String($("#" + orderType + "_asset_price").val())), currentAsset.decimals));
            var totalMTR = MRS.formatAmount(MRS.calculateOrderTotalMQT(quantityQNT, priceMQT, currentAsset.decimals), false, true);
        } catch (err) {
        ...
        var priceMQTPerWholeQNT = priceMQT.multiply(new BigInteger("" + Math.pow(10, currentAsset.decimals)));
        var description;
        var tooltipTitle;
        if (orderType == "buy") {
            description = $.t("buy_order_description", {
                "quantity": MRS.formatQuantity(quantityQNT, currentAsset.decimals, true),
                "asset_name": $("#asset_name").html().escapeHTML(),
                "amount": MRS.formatAmount(priceMQTPerWholeQNT),
                "symbol": MRS.constants.COIN_SYMBOL
            });
            tooltipTitle = $.t("buy_order_description_help", {
                "amount": MRS.formatAmount(priceMQTPerWholeQNT, false, true),
                "total": totalMTR,
                "symbol": MRS.constants.COIN_SYMBOL
            });
        } else {
        ...
        $("#asset_order_total").html(totalMTR + " " + MRS.constants.COIN_SYMBOL);
        ...
        $("#asset_order_price").val(priceMQT.toString());
    });

    MRS.forms.orderAsset = function () {
        var orderType = $("#asset_order_type").val();
        if (orderType == "placeBidOrder" && MRS.isShowFakeWarning()) {
            return MRS.composeFakeWarning($.t("asset"), $("#asset_order_asset").val());
        }
        return {
    ...
    MRS.forms.issueAsset = function ($modal) {
        var data = MRS.getFormData($modal.find("form:first"));
        ...
        } else if (!MRS.isNumericAccount(data.quantity)) {
            return {
    ...
    MRS.getAssetAccounts = function (assetId, height, success, error) {
        MRS.sendRequest("getAssetAccounts", {"asset": assetId, "height": height}, function (response) {
            if (response.errorCode) {
                error(response);
            } else {
                success(response);
    ...
    $("#asset_exchange_sidebar_group_context").on("click", "a", function (e) {
        e.preventDefault();
        var groupName = MRS.selectedContext.data("groupname");
    ...
    MRS.forms.assetExchangeChangeGroupName = function () {
    ...
    $("#asset_exchange_sidebar_context").on("click", "a", function (e) {
        e.preventDefault();
        var assetId = MRS.selectedContext.data("asset");
        var option = $(this).data("option");
        ...
        if (option == "add_to_group") {
            ...
            MRS.storageSelect("assets", [{
                ...
                asset = asset[0];
                $("#asset_exchange_group_title").html(MRS.escapeRespStr(asset.name));
                MRS.storageSelect("assets", [], function (error, assets) {
                    var groupNames = [];
                    $.each(assets, function (index, asset) {
    ...
    MRS.forms.assetExchangeGroup = function () {
    ...
    /* TRADE HISTORY PAGE */
    MRS.pages.trade_history = function () {
        MRS.sendRequest("getTrades+", {
            ...
            if (response.trades && response.trades.length) {
                if (response.trades.length > MRS.itemsPerPage) {
                ...
                var amountDecimals = MRS.getNumberOfDecimals(trades, "totalMQT", function(val) {
                    return MRS.formatAmount(MRS.calculateOrderTotalMQT(val.quantityQNT, val.priceMQT));
                });
                var rows = "";
                for (var i = 0; i < trades.length; i++) {
                    ...
                    trade.quantityQNT = new BigInteger(trade.quantityQNT);
                    trade.totalMQT = new BigInteger(MRS.calculateOrderTotalMQT(trade.priceMQT, trade.quantityQNT));
                    var type = (trade.buyerRS == MRS.accountRS ? "buy" : "sell");
                    rows += "<tr>" +
                        "<td><a href='#' data-goto-asset='" + MRS.escapeRespStr(trade.asset) + "'>" + MRS.escapeRespStr(trade.name) + "</a></td>" +
                        "<td>" + MRS.formatTimestamp(trade.timestamp) + "</td>" +
                        "<td>" + $.t(trade.tradeType) + "</td>" +
                        "<td class='numeric'>" + MRS.formatQuantity(trade.quantityQNT, trade.decimals, false, quantityDecimals) + "</td>" +
                        "<td class='asset_price numeric'>" + MRS.formatOrderPricePerWholeQNT(trade.priceMQT, trade.decimals, priceDecimals) + "</td>" +
                        "<td style='" + (type == "buy" ? "color:red" : "color:green") + "' class='numeric'>" + MRS.formatAmount(trade.totalMQT, false, false, amountDecimals) + "</td>" +
                        "<td>" + MRS.getAccountLink(trade, "buyer") + "</td>" +
                        "<td>" + MRS.getAccountLink(trade, "seller") + "</td>" +
                    "</tr>";
                }
                MRS.dataLoaded(rows);
            } else {
    ...
    MRS.pages.transfer_history = function () {
        MRS.sendRequest("getAssetTransfers+", {
            "account": MRS.accountRS,
            "includeAssetInfo": true,
            "firstIndex": MRS.pageNumber * MRS.itemsPerPage - MRS.itemsPerPage,
            "lastIndex": MRS.pageNumber * MRS.itemsPerPage
            ...
            if (response.transfers && response.transfers.length) {
                if (response.transfers.length > MRS.itemsPerPage) {
                    MRS.hasMorePages = true;
                    response.transfers.pop();
                }
                var transfers = response.transfers;
                var quantityDecimals = MRS.getNumberOfDecimals(transfers, "quantityQNT", function(val) {
                    return MRS.formatQuantity(val.quantityQNT, val.decimals);
                });
                var rows = "";
                for (var i = 0; i < transfers.length; i++) {
                    ...
                    var type = (transfer.recipientRS == MRS.accountRS ? "receive" : "send");
                    rows += "<tr>" +
                        "<td>" + MRS.getTransactionLink(transfer.assetTransfer) + "</td>" +
                        "<td><a href='#' data-goto-asset='" + MRS.escapeRespStr(transfer.asset) + "'>" + MRS.escapeRespStr(transfer.name) + "</a></td>" +
                        "<td>" + MRS.formatTimestamp(transfer.timestamp) + "</td>" +
                        "<td style='" + (type == "receive" ? "color:green" : "color:red") + "' class='numeric'>" + MRS.formatQuantity(transfer.quantityQNT, transfer.decimals, false, quantityDecimals) + "</td>" +
                        "<td>" + MRS.getAccountLink(transfer, "recipient") + "</td>" +
                        "<td>" + MRS.getAccountLink(transfer, "sender") + "</td>" +
                    "</tr>";
                }
                MRS.dataLoaded(rows);
            } else {
    ...
    MRS.pages.deletes_history = function () {
        MRS.sendRequest("getAssetDeletes+", {
            "account": MRS.accountRS,
            "includeAssetInfo": true,
            "firstIndex": MRS.pageNumber * MRS.itemsPerPage - MRS.itemsPerPage,
            "lastIndex": MRS.pageNumber * MRS.itemsPerPage
            ...
            if (response.deletes && response.deletes.length) {
                if (response.deletes.length > MRS.itemsPerPage) {
                    MRS.hasMorePages = true;
                    response.deletes.pop();
                }
                var deletes = response.deletes;
                var quantityDecimals = MRS.getNumberOfDecimals(deletes, "quantityQNT", function(val) {
                    return MRS.formatQuantity(val.quantityQNT, val.decimals);
                });
                var rows = "";
                for (var i = 0; i < deletes.length; i++) {
                    ...
                    rows += "<tr>" +
                        "<td>" + MRS.getTransactionLink(deletes[i].assetDelete) + "</td>" +
                        "<td><a href='#' data-goto-asset='" + MRS.escapeRespStr(deletes[i].asset) + "'>" + MRS.escapeRespStr(deletes[i].name) + "</a></td>" +
                        "<td>" + MRS.formatTimestamp(deletes[i].timestamp) + "</td>" +
                        "<td class='numeric'>" + MRS.formatQuantity(deletes[i].quantityQNT, deletes[i].decimals, false, quantityDecimals) + "</td>" +
                    "</tr>";
                }
                MRS.dataLoaded(rows);
            } else {
    ...
    /* MY ASSETS PAGE */
    MRS.pages.my_assets = function () {
        if (MRS.accountInfo.assetBalances && MRS.accountInfo.assetBalances.length) {
            var result = {
                "assets": [],
                "bid_orders": {},
                "ask_orders": {}
            ...
            var count = {
                "total_assets": MRS.accountInfo.assetBalances.length,
                "assets": 0,
                "ignored_assets": 0,
                "ask_orders": 0,
                "bid_orders": 0
            ...
            for (var i = 0; i < MRS.accountInfo.assetBalances.length; i++) {
                if (MRS.accountInfo.assetBalances[i].balanceQNT == "0") {
                    count.ignored_assets++;
                    if (MRS.checkMyAssetsPageLoaded(count)) {
                        MRS.myAssetsPageLoaded(result);
                    }
                    continue;
                }

                MRS.sendRequest("getAskOrders+", {
                    "asset": MRS.accountInfo.assetBalances[i].asset,
                    "firstIndex": 0,
                    "lastIndex": 1
                }, function (response, input) {
                    if (MRS.currentPage != "my_assets") {
                        return;
                    }

                    if (response.askOrders && response.askOrders.length) {
                        result.ask_orders[input.asset] = new BigInteger(response.askOrders[0].priceMQT);
                    } else {
                ...
                MRS.sendRequest("getBidOrders+", {
                    ...
                    if (response.bidOrders && response.bidOrders.length) {
                        result.bid_orders[input.asset] = new BigInteger(response.bidOrders[0].priceMQT);
                    } else {
                ...
                MRS.sendRequest("getAsset+", {
                    "asset": MRS.accountInfo.assetBalances[i].asset,
                    "_extra": {
                        "balanceQNT": MRS.accountInfo.assetBalances[i].balanceQNT
                    }
    ...
    MRS.myAssetsPageLoaded = function (result) {
        ...
        var quantityDecimals = MRS.getNumberOfDecimals(result.assets, "balanceQNT", function(asset) {
            return MRS.formatQuantity(asset.balanceQNT, asset.decimals);
        });
        var totalDecimals = MRS.getNumberOfDecimals(result.assets, "quantityQNT", function(asset) {
            return MRS.formatQuantity(asset.quantityQNT, asset.decimals);
        });
        var askDecimals = MRS.getNumberOfDecimals(result.assets, "ask", function(asset) {
            if (!asset.ask_orders || asset.ask_orders == -1) {
                return "";
            }
            return MRS.formatOrderPricePerWholeQNT(asset.ask_orders, asset.decimals);
        });
        var bidDecimals = MRS.getNumberOfDecimals(result.assets, "bid", function(asset) {
            if (!asset.bid_orders || asset.bid_orders == -1) {
                return "";
            }
            return MRS.formatOrderPricePerWholeQNT(asset.bid_orders, asset.decimals);
        });
        var valueDecimals = MRS.getNumberOfDecimals(result.assets, "bid", function(asset) {
            if (!asset.bid_orders || asset.bid_orders == -1) {
                return "";
            }
            var totalMQT = new BigInteger(MRS.calculateOrderTotalMQT(asset.balanceQNT, asset.bid_orders));
            return MRS.formatAmount(totalMQT);
        });
        for (var i = 0; i < result.assets.length; i++) {
            ...
            if (highestBidOrder != -1) {
                var totalMQT = new BigInteger(MRS.calculateOrderTotalMQT(asset.balanceQNT, highestBidOrder));
            }
            rows += "<tr data-asset='" + MRS.escapeRespStr(asset.asset) + "'>" +
                "<td><a href='#' data-goto-asset='" + MRS.escapeRespStr(asset.asset) + "'>" + MRS.escapeRespStr(asset.name) + "</a></td>" +
                "<td class='quantity numeric'>" + MRS.formatQuantity(asset.balanceQNT, asset.decimals, false, quantityDecimals) + "</td>" +
                "<td class='numeric'>" + MRS.formatQuantity(asset.quantityQNT, asset.decimals, false, totalDecimals) + "</td>" +
                "<td class='numeric'>" + percentageAsset + "%</td>" +
                "<td class='numeric'>" + (lowestAskOrder != -1 ? MRS.formatOrderPricePerWholeQNT(lowestAskOrder, asset.decimals, askDecimals) : "") + "</td>" +
                "<td class='numeric'>" + (highestBidOrder != -1 ? MRS.formatOrderPricePerWholeQNT(highestBidOrder, asset.decimals, bidDecimals) : "") + "</td>" +
                "<td class='numeric'>" + (highestBidOrder != -1 ? MRS.formatAmount(totalMQT, false, false, valueDecimals) : "") + "</td>" +
                "<td>" +
                    "<a href='#' class='btn btn-xs btn-default' data-toggle='modal' data-target='#transfer_asset_modal' data-asset='" + MRS.escapeRespStr(asset.asset) + "' data-name='" + MRS.escapeRespStr(asset.name) + "' data-decimals='" + MRS.escapeRespStr(asset.decimals) + "' data-action='transfer_asset'>" + $.t("transfer") + "</a>" +
                    "<a href='#' class='btn btn-xs btn-default' data-toggle='modal' data-target='#transfer_asset_modal' data-asset='" + MRS.escapeRespStr(asset.asset) + "' data-name='" + MRS.escapeRespStr(asset.name) + "' data-decimals='" + MRS.escapeRespStr(asset.decimals) + "' data-action='delete_shares'>" + $.t("delete_shares") + "</a>" +
                "</td>" +
            "</tr>";
        }
        MRS.dataLoaded(rows);
    ...
    MRS.incoming.my_assets = function () {
        MRS.loadPage("my_assets");
    };
    ...
    assetDistributionModal.on("show.bs.modal", function (e) {
        var $invoker = $(e.relatedTarget);
        var assetId = $invoker.data("asset");
        MRS.sendRequest("getAssetAccounts", {
            "asset": assetId,
            "lastIndex": MRS.state.maxAPIRecords
        }, function (response) {
            var rows = "";
            if (response.accountAssets) {
                ...
                for (var i = 0; i < response.accountAssets.length; i++) {
                    ...
                    var percentageAsset = MRS.calculatePercentage(account.quantityQNT, currentAsset.quantityQNT);
                    rows += "<tr><td>" + MRS.getAccountLink(account, "account", currentAsset.accountRS, "asset_issuer") + "</td><td>" + MRS.formatQuantity(account.quantityQNT, currentAsset.decimals) + "</td><td>" + percentageAsset + "%</td></tr>";
                }
            }
            var assetDistributionTable = $("#asset_distribution_table");
            assetDistributionTable.find("tbody").empty().append(rows);
            MRS.dataLoadFinished(assetDistributionTable);
        });
    });
    ...
    $("#transfer_asset_modal").on("show.bs.modal", function (e) {
        ...
        if (confirmedBalance == unconfirmedBalance) {
            availableAssetsMessage = " - " + $.t("available_qty", {
                "qty": MRS.formatQuantity(confirmedBalance, decimals)
            });
        } else {
            availableAssetsMessage = " - " + $.t("available_qty", {
                "qty": MRS.formatQuantity(unconfirmedBalance, decimals)
            }) + " (" + MRS.formatQuantity(confirmedBalance, decimals) + " " + $.t("total_lowercase") + ")";
        }
        $("#transfer_asset_available").html(availableAssetsMessage);
    });

    MRS.forms.transferAsset = function ($modal) {
        return transferOrDeleteShares($modal);
    };

    MRS.forms.deleteAssetShares = function ($modal) {
        return transferOrDeleteShares($modal);
    };
    ...
    function transferOrDeleteShares($modal) {
        ...
        if (!data.quantity) {
            return {
                "error": $.t("error_not_specified", {
                    "name": MRS.getTranslatedFieldName("quantity").toLowerCase()
                }).capitalize()
            };
        }

        if (!MRS.showedFormWarning) {
            if (MRS.settings["asset_transfer_warning"] && MRS.settings["asset_transfer_warning"] != 0) {
                if (new Big(data.quantity).cmp(new Big(MRS.settings["asset_transfer_warning"])) > 0) {
                    MRS.showedFormWarning = true;
                    return {
                        "error": $.t("error_max_asset_transfer_warning", {
                            "qty": String(MRS.settings["asset_transfer_warning"]).escapeHTML()
                        })
                    };
                }
            }
        ...
        try {
            data.quantityQNT = MRS.convertToQNT(data.quantity, data.decimals);
        } catch (e) {
            return {
    ...
    MRS.forms.transferAssetComplete = function () {
        MRS.loadPage("my_assets");
    };
    ...
    $("body").on("click", "a[data-goto-asset]", function (e) {
        ...
        viewingAsset = true;
        MRS.goToAsset($(this).data("goto-asset"));
    });

    MRS.goToAsset = function (asset) {
        assetSearch = false;
    ...
    /* OPEN ORDERS PAGE */
    MRS.pages.open_orders = function () {
        var loaded = 0;
        MRS.getOpenOrders("ask", function () {
            loaded++;
            if (loaded == 2) {
                MRS.pageLoaded();
            }
        ...
        MRS.getOpenOrders("bid", function () {
            loaded++;
            if (loaded == 2) {
                MRS.pageLoaded();
            }
    ...
    MRS.getOpenOrders = function (type, callback) {
        ...
        MRS.sendRequest(getAccountCurrentOrders, {
            "account": MRS.account,
            "firstIndex": 0,
            "lastIndex": 100
        }, function (response) {
            if (response[accountOrders] && response[accountOrders].length) {
                ...
                for (var i = 0; i < response[accountOrders].length; i++) {
                    MRS.sendRequest("getAsset+", {
                        ...
                        if (MRS.currentPage != "open_orders") {
                            return;
                        }
                        ...
                        if (nrOrders == response[accountOrders].length) {
                            MRS.openOrdersLoaded(response[accountOrders], lowercase, callback);
                        }
                    });
                }
            } else {
                MRS.openOrdersLoaded([], lowercase, callback);
            }
        });
    };

    MRS.openOrdersLoaded = function (orders, type, callback) {
        var openOrdersTable = $("#open_" + type + "_orders_table");
        if (!orders.length) {
            $("#open_" + type + "_orders_table tbody").empty();
            MRS.dataLoadFinished(openOrdersTable);
            callback();
            return;
        }
        ...
        for (var i = 0; i < orders.length; i++) {
            ...
            completeOrder.priceMQT = new BigInteger(completeOrder.priceMQT);
            completeOrder.quantityQNT = new BigInteger(completeOrder.quantityQNT);
            completeOrder.totalMQT = new BigInteger(MRS.calculateOrderTotalMQT(completeOrder.quantityQNT, completeOrder.priceMQT));
            rows += "<tr data-order='" + MRS.escapeRespStr(completeOrder.order) + "'><td><a href='#' data-goto-asset='" + MRS.escapeRespStr(completeOrder.asset) + "'>" + MRS.escapeRespStr(completeOrder.assetName) + "</a></td><td>" + MRS.formatQuantity(completeOrder.quantityQNT, completeOrder.decimals) + "</td><td>" + MRS.formatOrderPricePerWholeQNT(completeOrder.priceMQT, completeOrder.decimals) + "</td><td>" + MRS.formatAmount(completeOrder.totalMQT) + "</td><td class='cancel'><a href='#' data-toggle='modal' data-target='#cancel_order_modal' data-order='" + MRS.escapeRespStr(completeOrder.order) + "' data-type='" + type + "'>" + $.t("cancel") + "</a></td></tr>";
        }
    ...
    MRS.incoming.open_orders = function (transactions) {
        if (MRS.hasTransactionUpdates(transactions)) {
            MRS.loadPage("open_orders");
        }
    };
    ...
    MRS.forms.cancelOrder = function ($modal) {
        var data = MRS.getFormData($modal.find("form:first"));
    ...
    MRS.forms.cancelOrderComplete = function (response, data) {
            ...
            if (response.accountAssets) {
                if (response.accountAssets.length > 0) {
                    ...
                    $.each(response.accountAssets, function (key, asset) {
                        ...
                        var $option = $('<option value="' + idString + '">' + MRS.escapeRespStr(asset.name) + '</option>');
    ...
    MRS.pages.approval_requests_asset = function () {
        MRS.buildApprovalRequestAssetNavi();
        if (selectedApprovalAsset != "") {
            ...
            MRS.sendRequest("getAssetPhasedTransactions", params, function (response) {
                var rows = "";
    ...
    $('#approve_asset_select').on('change', function () {
        ...
        MRS.loadPage("approval_requests_asset");
    });
...
/*
    MRS.setup.asset_exchange = function () {
        ...
        var options = {
            ...
            "page": 'asset_exchange',
            "desiredPosition": 30,
            "depends": { tags: [ MRS.constants.API_TAGS.AE ] }
        };
