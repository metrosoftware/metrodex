...
var MRS = (function(MRS, $) {
    ...
    $("#mobile_settings_modal").on("show.bs.modal", function() {
        ...
        if (isOffline) {
            ...
            $(".info_message").html($.t("remote_node_url", {url: MRS.getRemoteNodeUrl()}));
        ...
        if (MRS.isEnableMobileAppSimulation()) {
            if (MRS.mobileSettings.is_simulate_app) {
        ...
        if (MRS.mobileSettings.is_testnet) {
            $("#mobile_is_testnet").prop('checked', true);
        } else {
            $("#mobile_is_testnet").prop('checked', false);
        }
        $("#mobile_remote_node_address").val(MRS.mobileSettings.remote_node_address);
        $("#mobile_remote_node_port").val(MRS.mobileSettings.remote_node_port);
        if (MRS.mobileSettings.is_remote_node_ssl) {
            $("#mobile_is_remote_node_ssl").prop('checked', true);
        } else {
            $("#mobile_is_remote_node_ssl").prop('checked', false);
        }
        $("#mobile_validators_count").val(MRS.mobileSettings.validators_count);
        $("#mobile_bootstrap_nodes_count").val(MRS.mobileSettings.bootstrap_nodes_count);
    });

    MRS.forms.setMobileSettings = function() {
        MRS.mobileSettings.is_check_remember_me = $("#mobile_is_check_remember_me").prop('checked');
        MRS.mobileSettings.is_store_remembered_passphrase = $("#mobile_is_store_remembered_passphrase").prop('checked');
        MRS.mobileSettings.is_simulate_app = $("#mobile_is_simulate_app").prop('checked');
        MRS.mobileSettings.is_testnet = $("#mobile_is_testnet").prop('checked');
        MRS.mobileSettings.remote_node_address = $("#mobile_remote_node_address").val();
        ...
        MRS.mobileSettings.remote_node_port = parseInt(remoteNodePort);
        MRS.mobileSettings.is_remote_node_ssl = $("#mobile_is_remote_node_ssl").prop('checked');
        ...
        MRS.mobileSettings.validators_count = count;
        ...
        MRS.mobileSettings.bootstrap_nodes_count = count;
        MRS.setJSONItem("mobile_settings", MRS.mobileSettings);
