...
var MRS = (function(MRS, $, undefined) {
    ...
    modal.on("show.bs.modal", function() {
		...
		if ($visible_modal.length) {
			if ($visible_modal.hasClass("locked")) {
				...
				MRS.unlockForm($visible_modal, $btn, true);
			} else {
		...
		MRS.initAdvancedModalFormValues($(this));
	...
	modal.on("shown.bs.modal", function() {
		...
		MRS.showedFormWarning = false; //maybe not the best place... we assume forms are only in modals?
	...
	MRS.showModalError = function(errorMessage, $modal) {
		var $btn = $modal.find("button.btn-primary:not([data-dismiss=modal], .ignore)");
		$modal.find("button").prop("disabled", false);
		$modal.find(".error_message").html(MRS.escapeRespStr(errorMessage)).show();
		$btn.button("reset");
		$modal.modal("unlock");
	};

	MRS.closeModal = function($modal) {
		if (!$modal) {
	...
	MRS.isShowFakeWarning = function() {
		if (MRS.settings.fake_entity_warning != "1") {
			return false;
		}
		return !isFakeWarningDisplayed;
    };

	MRS.composeFakeWarning = function (entity, id) {
