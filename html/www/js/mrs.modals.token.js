...
var MRS = (function(MRS, $) {
	...
	MRS.forms.decodeToken = function() {
		return {
			data: {
	...
	MRS.forms.decodeTokenComplete = function(response) {
		$("#token_modal").find(".error_message").hide();

		if (response.valid) {
			$("#decode_token_output").html($.t("success_valid_token", {
				"account_link": MRS.getAccountLink(response, "account"),
				"timestamp": MRS.formatTimestamp(response.timestamp)
	...
	MRS.forms.decodeTokenError = function() {
    ...
    $("#generate_token_button").click(function (e) {
		var data = MRS.getFormData($("#generate_token_form"));
        ...
        if (!MRS.rememberPassword) {
			...
			var accountId = MRS.getAccountId(secretPhrase);
			if (accountId != MRS.account && !isOffline) {
        ...
        var token = MRS.generateToken(website, secretPhrase);
        ...
        MRS.generateQRCode("#generate_token_output_qr_code", token, 14);
        outputQrCodeContainer.show();
        e.preventDefault();
    });

    MRS.setTokenPassword = function(password) {
        _password = password;
    };
