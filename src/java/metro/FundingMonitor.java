 ...
 * <p>
 * MTR and ASSET balances can be monitored.  If a balance falls below the threshold, a transaction
...
public final class FundingMonitor {
    ...
    private static final int MAX_MONITORS = Metro.getIntProperty("metro.maxNumberOfMonitors");
     ...
     * @param   holdingId           Asset identifier, ignored for MTR monitor
    ...
    public static boolean startMonitor(HoldingType holdingType, long holdingId, String property,
        ...
        // instead of during MRS initialization so we don't start the monitor thread if it
        // won't be used.
        //
        init();
    ...
    private static synchronized void init() {
        ...
        try {
            ...
            Metro.getBlockchainProcessor().addListener(new BlockEventHandler(), BlockchainProcessor.Event.BLOCK_PUSHED);
            //
            // All done
            //
            started = true;
                ...
                while (true) {
                    ...
                    while ((monitoredAccount = pendingEvents.poll()) != null) {
                        try {
                            ...
                            if (Metro.getBlockchain().getHeight() - monitoredAccount.height < monitoredAccount.interval) {
                                ...
                                switch (monitoredAccount.monitor.holdingType) {
                                    case MTR:
                                        processMetroEvent(monitoredAccount, targetAccount, fundingAccount);
                                        break;
                                    case ASSET:
     ...
     * @throws MetroException                Unable to create transaction
     */
    private static void processMetroEvent(MonitoredAccount monitoredAccount, Account targetAccount, Account fundingAccount)
        ...
        if (targetAccount.getBalanceMQT() < monitoredAccount.threshold) {
            ...
            if (Math.addExact(monitoredAccount.amount, transaction.getFeeMQT()) > fundingAccount.getUnconfirmedBalanceMQT()) {
                ...
                monitoredAccount.height = Metro.getBlockchain().getHeight();
                Logger.logDebugMessage(String.format("%s funding transaction %s for %f %s submitted from %s to %s",
                        Constants.COIN_SYMBOL, transaction.getStringId(), (double)monitoredAccount.amount / Constants.ONE_MTR,
    ...
    private static final class AccountEventHandler implements Listener<Account> {
        ...
        public void notify(Account account) {
            ...
            long balance = account.getBalanceMQT();
            //
