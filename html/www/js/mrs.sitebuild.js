...
var MRS = (function(MRS, $) {
    ...
    MRS.loadLockscreenHTML = function(path) {
    ...
    MRS.loadHeaderHTML = function(path) {
    	jQuery.ajaxSetup({ async: false });
    	$.get(path, '', function (data) { $("body").prepend(data); });
    	jQuery.ajaxSetup({ async: true });
    };

    MRS.loadSidebarHTML = function(path) {
    	jQuery.ajaxSetup({ async: false });
    	$.get(path, '', function (data) { $("#sidebar").append(data); });
    	jQuery.ajaxSetup({ async: true });
    };

    MRS.loadSidebarContextHTML = function(path) {
    	jQuery.ajaxSetup({ async: false });
    	$.get(path, '', function (data) { $("body").append(data); });
    	jQuery.ajaxSetup({ async: true });
    };

    MRS.loadPageHTML = function(path) {
    	jQuery.ajaxSetup({ async: false });
        MRS.asyncLoadPageHTML(path);
    	jQuery.ajaxSetup({ async: true });
    };

    MRS.asyncLoadPageHTML = function(path) {
    	$.get(path, '', function (data) { $("#content").append(data); });
    };

    MRS.loadModalHTML = function(path) {
    	jQuery.ajaxSetup({ async: false });
    	$.get(path, '', function (data) { $("body").append(data); });
    	jQuery.ajaxSetup({ async: true });
    };

    MRS.loadPageHTMLTemplates = function(options) {
    ...
    MRS.loadModalHTMLTemplates = function() {
        jQuery.ajaxSetup({ async: false });
    ...
    MRS.preloadModalUIElements = function() {
        jQuery.ajaxSetup({ async: false });
    ...
    MRS.initModalUIElement = function($modal, selector, elementName, context) {
    ...
    MRS.initSidebarMenu = function() {
        $("#sidebar_menu").html("");
    };

    MRS.addSimpleSidebarMenuItem = function(options) {
        if (!MRS.isApiEnabled(options.depends)) {
            return;
        }
    ...
    MRS.addTreeviewSidebarMenuItem = function(options) {
        if (!MRS.isApiEnabled(options.depends)) {
            return;
        }
    ...
    MRS.appendMenuItemToTSMenuItem = function(itemId, options) {
        if (!MRS.isApiEnabled(options.depends)) {
            return;
        }
    ...
    MRS.appendSubHeaderToTSMenuItem = function(itemId, options) {
        if (!MRS.isApiEnabled(options.depends)) {
            return;
        }
        var parentMenu = $('#' + itemId + ' ul.treeview-menu');
    ...
    MRS.getUrlParameter = function (param) {
