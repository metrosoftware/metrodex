 ...
 * has not been received within metro.apiEventTimeout seconds.
 *
 * The maximum number of event users is specified by metro.apiMaxEventUsers.
 */
class EventListener implements Runnable, AsyncListener, TransactionalDb.TransactionCallback {

    /** Maximum event users */
    static final int maxEventUsers = Metro.getIntProperty("metro.apiMaxEventUsers");

    /** Event registration timeout (seconds) */
    static final int eventTimeout = Math.max(Metro.getIntProperty("metro.apiEventTimeout"), 15);

    /** Blockchain processor */
    static final BlockchainProcessor blockchainProcessor = Metro.getBlockchainProcessor();

    /** Transaction processor */
    static final TransactionProcessor transactionProcessor = Metro.getTransactionProcessor();
    ...
    /** Metro event listeners */
    private final List<MetroEventListener> metroEventListeners = new ArrayList<>();
     ...
     *
     * Metro event listeners will be added for the specified events
     *
     * @param   eventRegistrations      List of Metro event registrations
     ...
     *
     * @param   eventRegistrations      Metro event registrations
     * @throws  EventListenerException  Invalid Metro event
     */
    void addEvents(List<EventRegistration> eventRegistrations) throws EventListenerException {
        ...
        try {
            ...
            for (EventRegistration event : eventRegistrations) {
                ...
                Iterator<MetroEventListener> it = metroEventListeners.iterator();
                while (it.hasNext()) {
                ...
                if (addListener) {
                    MetroEventListener listener = new MetroEventListener(event);
                    listener.addListener();
                    metroEventListeners.add(listener);
                }
            }
        } finally {
            lock.unlock();
    ...
    void removeEvents(List<EventRegistration> eventRegistrations) {
        ...
        try {
            ...
            //
            if (metroEventListeners.isEmpty())
                deactivateListener();
        } finally {
            lock.unlock();
        }
    ...
    void deactivateListener() {
        ...
        try {
            ...
            //
            metroEventListeners.forEach(MetroEventListener::removeListener);
        } finally {
            lock.unlock();
        }
     ...
     */
    private class MetroEventListener {

        /** Event handler */
        private final MetroEventHandler eventHandler;

        /**
         * Create the Metro event listener
         *
         * @param   eventRegistration           Event registration
         * @throws  EventListenerException      Invalid event
         */
        public MetroEventListener(EventRegistration eventRegistration) throws EventListenerException {
        ...
        /**
         * Add the Metro listener for this event
         */
        ...
        /**
         * Remove the Metro listener for this event
         */
        ...
        /**
         * Return the hash code for this Metro event listener
         *
         * @return                  Hash code
         */
        @Override
        ...
        /**
         * Check if two Metro events listeners are equal
         *
        ...
        public boolean equals(Object obj) {
            return (obj != null && (obj instanceof MetroEventListener) &&
                    eventHandler.equals(((MetroEventListener)obj).eventHandler));
        }

        /**
         * Metro listener event handler
         */
        private abstract class MetroEventHandler {
            ...
            /**
             * Create the Metro event handler
             *
             * @param   eventRegistration   Event registration
             */
            public MetroEventHandler(EventRegistration eventRegistration) {
            ...
            @Override
            public boolean equals(Object obj) {
                return (obj != null && (obj instanceof MetroEventHandler) &&
                                        owner == ((MetroEventHandler)obj).owner &&
                                        accountId == ((MetroEventHandler)obj).accountId &&
                                        event == ((MetroEventHandler)obj).event);
            }
        }
         ...
         * Peer event handler
         */
        private class PeerEventHandler extends MetroEventHandler implements Listener<Peer> {
         ...
         */
        private class BlockEventHandler extends MetroEventHandler implements Listener<Block> {
         ...
         */
        private class TransactionEventHandler extends MetroEventHandler implements Listener<List<? extends Transaction>> {
         ...
         */
        private class LedgerEventHandler extends MetroEventHandler implements Listener<LedgerEntry> {
    ...
    static class EventRegistration {
        ...
        /**
         * Return the Metro listener event
         *
         * @return                  Metro listener event
         */
        public Enum<? extends Enum> getEvent() {
            return event;
        }
