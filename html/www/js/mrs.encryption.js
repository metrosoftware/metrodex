...
var MRS = (function (MRS, $) {
	...
	MRS.resetEncryptionState = function () {
		_password = null;
	...
	MRS.generatePublicKey = function(secretPhrase) {
		if (!secretPhrase) {
		...
		return MRS.getPublicKey(converters.stringToHexString(secretPhrase));
	};

	MRS.getPublicKey = function(id, isAccountId) {
		if (isAccountId) {
            var publicKey = "";
	...
	MRS.getPrivateKey = function(secretPhrase) {
	...
	MRS.getAccountId = function(secretPhrase, isRsFormat) {
		return MRS.getAccountIdFromPublicKey(MRS.getPublicKey(converters.stringToHexString(secretPhrase)), isRsFormat);
	};

	MRS.getAccountIdFromPublicKey = function(publicKey, isRsFormat) {
	...
	MRS.getEncryptionKeys = function (options, secretPhrase){
		if (!options.sharedKey) {
			if (!options.privateKey) {
				...
				options.privateKey = converters.hexStringToByteArray(MRS.getPrivateKey(secretPhrase));
			}
			...
			if (!options.publicKey) {
				...
				try {
					options.publicKey = converters.hexStringToByteArray(MRS.getPublicKey(options.account, true));
				} catch (err) {
					var metroAddress = new MetroAddress();

					if (!metroAddress.set(options.account)) {
						throw {
    ...
    MRS.encryptNote = function(message, options, secretPhrase) {
		try {
			options = MRS.getEncryptionKeys(options, secretPhrase);
	...
	MRS.decryptNote = function(message, options, secretPhrase) {
		try {
			if (!options.sharedKey) {
				if (!options.privateKey) {
					if (!secretPhrase) {
	...
	MRS.signBytes = function(message, secretPhrase) {
		if (!secretPhrase) {
			if (MRS.rememberPassword) {
				secretPhrase = _password;
			} else {
	...
	MRS.verifySignature = function(signature, message, publicKey, callback) {
	...
	MRS.setEncryptionPassword = function(password) {
		_password = password;
	};

	MRS.setDecryptionPassword = function(password) {
		_decryptionPassword = password;
	};

	MRS.tryToDecryptMessage = function(message) {
		...
		try {
			if (!message.attachment.encryptedMessage.data) {
				...
				var decoded = MRS.decryptNote(message.attachment.encryptedMessage.data, {
					"nonce": message.attachment.encryptedMessage.nonce,
					"account": (message.recipient == MRS.account ? message.sender : message.recipient),
	...
	MRS.tryToDecrypt = function(transaction, fields, account, options) {
		var showDecryptionForm = false;
		if (!options) {
			options = {};
		}
		var nrFields = Object.keys(fields).length;
		var formEl = (options.formEl ? MRS.escapeRespStr(options.formEl) : "#transaction_info_output_bottom");
		var outputEl = (options.outputEl ? MRS.escapeRespStr(options.outputEl) : "#transaction_info_output_bottom");
		var output = "";
			...
			$.each(fields, function(key, title) {
				...
				if (encrypted) {
					...
					try {
                        ...
                        data = MRS.decryptNote(encrypted, decryptOptions);
					} catch (err) {
						if (err.errorCode && err.errorCode == 1) {
							...
							if (title) {
								var translatedTitle = MRS.getTranslatedFieldName(title).toLowerCase();
								if (!translatedTitle) {
									translatedTitle = MRS.escapeRespStr(title).toLowerCase();
								}
		...
		if (showDecryptionForm) {
			...
			MRS.removeDecryptionForm();
			$(outputEl).append(output).show();
		}
	};

	MRS.removeDecryptionForm = function($modal) {
    ...
    var formatMessageArea = function (title, nrFields, data, options, transaction) {
		...
		if (data.sharedKey) {
			...
			if (!MRS.isTextMessage(transaction) && transaction.block) {
				downloadLink = MRS.getMessageDownloadLink(transaction.transaction, data.sharedKey) + "<br>";
			}
		}
        return "<div style='" + outputStyle + "'>" + label + "<div>" + msg + "</div>" + sharedKeyField + downloadLink + "</div>";
    };

    MRS.decryptNoteFormSubmit = function() {
		...
		var accountId = MRS.getAccountId(password);
		if (accountId != MRS.account && !useSharedKey) {
		...
		$.each(_encryptedNote.fields, function(key, title) {
			...
			if (encrypted) {
				...
				try {
                    ...
                    data = MRS.decryptNote(encrypted, options, password);
		...
		MRS.removeDecryptionForm();
		var outputEl = (_encryptedNote.options.outputEl ? MRS.escapeRespStr(_encryptedNote.options.outputEl) : "#transaction_info_output_bottom");
		$(outputEl).append(output).show();
		_encryptedNote = null;
		if (rememberPassword) {
			_decryptionPassword = password;
	...
	MRS.decryptAllMessages = function(messages, password, sharedKey) {
		var useSharedKey = false;
		if (!password) {
			if (!sharedKey) {
				throw {
		...
		for (var i = 0; i < messages.length; i++) {
			...
			if (message.attachment.encryptedMessage && !_decryptedTransactions[message.transaction]) {
				try {
					var otherUser = (message.sender == MRS.account ? message.recipient : message.sender);
					var options = {};
					if (useSharedKey) {
						options.sharedKey = converters.hexStringToByteArray(sharedKey);
					} else {
                    ...
                    var decoded = MRS.decryptNote(message.attachment.encryptedMessage.data, options, password);
    ...
    MRS.encryptDataRoof = function(data, options) {
	...
	MRS.decryptDataRoof = function(data, options) {
    ...
    MRS.sharedSecretToSharedKey = function (sharedSecret, nonce) {
        for (var i = 0; i < 32; i++) {
    ...
    MRS.getSharedKey = function (privateKey, publicKey, nonce) {
		var sharedSecret = getSharedSecret(privateKey, publicKey);
        return MRS.sharedSecretToSharedKey(sharedSecret, nonce);
	};

	MRS.encryptFile = function(file, options, callback) {
		var r;
		try {
			r = new FileReader();
		} catch(e) {
    ...
    MRS.encryptMessage = function(MRS, text, senderSecretPhrase, recipientPublicKey, isMessageToSelf) {
        var encrypted = MRS.encryptNote(text, {
