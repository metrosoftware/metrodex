...
final class TransactionDb {
    ...
    static TransactionImpl loadTransaction(Connection con, ResultSet rs) throws MetroException.NotValidException {
        try {
            ...
            long amountMQT = rs.getLong("amount");
            long feeMQT = rs.getLong("fee");
            ...
            TransactionImpl.BuilderImpl builder = new TransactionImpl.BuilderImpl(version, null,
                    amountMQT, feeMQT, deadline, transactionType.parseAttachment(buffer))
                    .timestamp(timestamp)
