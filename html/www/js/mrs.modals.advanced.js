...
var MRS = (function(MRS, $, undefined) {
	...
	MRS.setAdvancedModalPassword = function (password) {
		_password = password;
	};

	MRS.showRawTransactionModal = function(transaction) {
        if (transaction.unsignedTransactionBytes && !transaction.transactionBytes) {
            ...
            MRS.generateQRCode("#raw_transaction_modal_unsigned_bytes_qr_code", transaction.unsignedTransactionBytes, 14);
        ...
        if (transaction.transactionJSON) {
            ...
            if (window.URL) {
                ...
                try {
                    ...
                    MRS.logConsole("Desktop Application in Java 8 does not support createObjectURL");
                    downloadLink.hide();
				}
            } else {
                downloadLink.hide();
    ...
    $(".qr_code_reader_link").click(function(e) {
        ...
        MRS.scanQRCode(readerId, function(data) {
            $("#" + outputId).val(data);
        });
    });
    ...
    MRS.forms.broadcastTransaction = function(modal) {
        ...
        if (modal.attr('id') == "transaction_json_modal") {
            data = MRS.getFormData($("#broadcast_json_form"));
        } else {
            data = MRS.getFormData(modal.find("form:first"));
        }
	...
	MRS.initAdvancedModalFormValues = function($modal) {
		...
		if (type != undefined && subType != undefined) {
			if (MRS.transactionTypes[type]["subTypes"][subType]["serverConstants"]["isPhasingSafe"] == true) {
		...
		if (MRS.accountInfo.phasingOnly) {
			minDuration = MRS.accountInfo.phasingOnly.minDuration;
			maxDuration = MRS.accountInfo.phasingOnly.maxDuration;
		}

		if (maxDuration == 0) {
			maxDuration = MRS.constants.SERVER.maxPhasingDuration;
		}
		...
		var context = {
			...
			initBlockHeight: MRS.lastBlockHeight + Math.round(minDuration + (maxDuration - minDuration) / 2),
			changeHeightBlocks: 500
		};
		var $elems = MRS.initModalUIElement($modal, '.phasing_finish_height_group', 'block_height_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);

		$elems = MRS.initModalUIElement($modal, '.mandatory_approval_finish_height_group', 'block_height_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		context = {
			...
			inputName: "phasingQuorumMTR",
			addonText: "MTR",
			addonI18n: "mtr_unit"
		};
		$elems = MRS.initModalUIElement($modal, '.approve_transaction_amount_mtr', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		$elems = MRS.initModalUIElement($modal, '.approve_transaction_asset_quantity', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		$elems = MRS.initModalUIElement($modal, '.add_approval_whitelist_group', 'multi_accounts_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		$elems = MRS.initModalUIElement($modal, '.approve_min_balance_model_group', 'min_balance_model_modal_ui_element', context);
		...
		context['inputName'] = 'phasingMinBalanceMTR';
		context['addonText'] = 'MTR';
		context['addonI18n'] = 'mtr_unit';
		$elems = MRS.initModalUIElement($modal, '.approve_min_balance_mtr', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();
		...
		$elems = MRS.initModalUIElement($modal, '.approve_min_balance_asset_quantity', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();
		...
		$elems = MRS.initModalUIElement($modal, '.approve_holding_asset', 'add_asset_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems = MRS.initModalUIElement($modal, '.approve_holding_asset_optional', 'add_asset_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();
		...
		MRS.initModalUIElement($modal, '.hash_algorithm_model_group', 'hash_algorithm_model_modal_ui_element', context);
	...
	MRS.forms.broadcastTransactionComplete = function() {
		$("#parse_transaction_form").find(".error_message").hide();
        $("#transaction_json_modal").modal("hide");
	};

	MRS.forms.parseTransactionComplete = function(response) {
		$("#parse_transaction_form").find(".error_message").hide();
        var details = $.extend({}, response);
        if (response.attachment) {
            delete details.attachment;
        }
        $("#parse_transaction_output_table").find("tbody").empty().append(MRS.createInfoTable(details, true));
		$("#parse_transaction_output").show();
	};

	MRS.forms.parseTransactionError = function() {
		$("#parse_transaction_output_table").find("tbody").empty();
		$("#parse_transaction_output").hide();
	};

	MRS.forms.calculateFullHashComplete = function(response) {
		$("#calculate_full_hash_form").find(".error_message").hide();
		$("#calculate_full_hash_output_table").find("tbody").empty().append(MRS.createInfoTable(response, true));
		$("#calculate_full_hash_output").show();
	};

	MRS.forms.calculateFullHashError = function() {
	...
	function updateSignature(signature) {
		...
		MRS.generateQRCode("#transaction_signature_qr_code", signature, 8);
		$("#signature_output").show();
	}

	MRS.forms.signTransactionComplete = function(response) {
    ...
    MRS.forms.signTransaction = function() {
        var data = MRS.getFormData($("#sign_transaction_form"));
		if (data.unsignedTransactionBytes && !data.validate) {
			MRS.logConsole("Sign transaction locally");
			var output = {};
			var secretPhrase = (MRS.rememberPassword ? _password : data.secretPhrase);
			var isOffline = $(".mobile-offline").val();
			if (MRS.getAccountId(secretPhrase) == MRS.account || isOffline) {
				try {
					var signature = MRS.signBytes(data.unsignedTransactionBytes, converters.stringToHexString(secretPhrase));
