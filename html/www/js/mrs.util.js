...
var MRS = (function (MRS, $, undefined) {
    MRS.formatVolume = function (volume) {
    ...
    MRS.formatWeight = function (weight) {
		var digits = [],
			formattedWeight = "",
			i;
		do {
    ...
    MRS.formatOrderPricePerWholeQNT = function (price, decimals, zeroPad) {
		price = MRS.calculateOrderPricePerWholeQNT(price, decimals, true);
		return MRS.format(price, false, zeroPad);
	};

    MRS.calculateOrderPricePerWholeQNT = function (price, decimals, returnAsObject) {
		if (typeof price != "object") {
			price = new BigInteger(String(price));
		}

		return MRS.convertToMTR(price.multiply(new BigInteger("" + Math.pow(10, decimals))), returnAsObject);
	};

    MRS.calculatePricePerWholeQNT = function (price, decimals) {
		price = String(price);
    ...
    function calculateOrderTotalImpl(quantityQNT, priceMQT) {
        if (typeof quantityQNT != "object") {
            quantityQNT = new BigInteger(String(quantityQNT));
        }
        if (typeof priceMQT != "object") {
            priceMQT = new BigInteger(String(priceMQT));
        }
        return quantityQNT.multiply(priceMQT);
    }

    MRS.calculateOrderTotalMQT = function (quantityQNT, priceMQT) {
        return calculateOrderTotalImpl(quantityQNT, priceMQT).toString();
    };

    MRS.calculateOrderTotal = function (quantityQNT, priceMQT) {
        return MRS.convertToMTR(calculateOrderTotalImpl(quantityQNT, priceMQT));
    };

    MRS.calculatePercentage = function (a, b, rounding_mode) {
		if (String(b) == "0") {
            return "0";
        }
		...
		Big.RM = 1;
		return MRS.format(result.toString());
	};

    MRS.convertToMTR = function (amount, returnAsObject) {
        var negative = "";
        var mantissa = "";
    ...
    MRS.amountToPrecision = function (amount, decimals) {
    ...
    MRS.convertToMQT = function (currency) {
    ...
    MRS.convertToQNTf = function (quantity, decimals, returnAsObject) {
    ...
    MRS.convertToQNT = function (quantity, decimals) {
		quantity = String(quantity);
    ...
    var zeros = "00000000";
    MRS.format = function (params, no_escaping, zeroPad) {
        var amount;
        var mantissa;
    ...
    MRS.formatQuantity = function (quantity, decimals, no_escaping, zeroPad) {
		return MRS.format(MRS.convertToQNTf(quantity, decimals, true), no_escaping, zeroPad);
	};

    MRS.formatAmount = function (amount, round, no_escaping, zeroPad) {
        ...
        if (typeof amount == "object") {
            var params = MRS.convertToMTR(amount, true);

            negative = params.negative;
            amount = params.amount;
            mantissa = params.mantissa;
        } else {
            //rounding only applies to non-mqt
            if (round) {
        ...
        if (MRS.settings) {
            ...
            var maxLength = parseInt(MRS.settings.max_mtr_decimals) + offset;
    ...
    MRS.getTransactionsAmountDecimals = function(transactions) {
        var decimals = {};
   		decimals.amount = MRS.getNumberOfDecimals(transactions, "amountMQT", function (transaction) {
   			return MRS.formatAmount(transaction.amountMQT);
   		});
   		decimals.fee = MRS.getNumberOfDecimals(transactions, "feeMQT", function (transaction) {
   			return MRS.formatAmount(transaction.feeMQT);
   		});
        return decimals;
   	};

    MRS.getNumberOfDecimals = function(rows, key, callback) {
        var locale = MRS.getLocale();
        var decimals = 0;
    ...
    MRS.fromEpochTime = function (epochTime) {
        if (!MRS.constants || MRS.constants.EPOCH_BEGINNING == 0) {
            throw "undefined epoch beginning";
        }
        return epochTime + MRS.constants.EPOCH_BEGINNING - 500;
    };

    MRS.toEpochTime = function (currentTime) {
    ...
    MRS.formatTimestamp = function (timestamp, date_only, isAbsoluteTime) {
		...
		if (typeof timestamp == "object") {
            ...
            date = new Date(MRS.fromEpochTime(timestamp));
		}
		...
		if (!isNaN(date) && typeof(date.getFullYear) == 'function') {
            ...
            if (!date_only) {
                ...
                if (!MRS.settings || MRS.settings["24_hour_format"] == "0") {
                    if (originalHours == 0) {
                        hours += 12;
    ...
    MRS.getBlockHeightMoment = function(height) {
        if (!height || !MRS.lastBlockHeight || !MRS.averageBlockGenerationTime) {
            return "-";
        }
        var heightDiff = height - MRS.lastBlockHeight;
        return moment().add(heightDiff * MRS.averageBlockGenerationTime, 'milliseconds');
    };

    MRS.getBlockHeightTimeEstimate = function(height) {
        var heightMoment = MRS.getBlockHeightMoment(height);
        if (heightMoment == "-") {
            return "-";
        }
    ...
    MRS.baseTargetPercent = function(block) {
        if (block) {
            return Math.round(block.baseTarget / MRS.constants.INITIAL_BASE_TARGET * 100)
        } else {
            return 0;
        }
    };

    MRS.convertToHex16 = function (str) {
		var hex, i;
		var result = "";
    ...
    MRS.convertFromHex16 = function (hex) {
		var j;
    ...
    MRS.convertFromHex8 = function (hex) {
    ...
    MRS.convertToHex8 = function (str) {
		var hex = '';
    ...
    MRS.getFormData = function ($form, unmodified) {
    ...
    MRS.mergeMaps = function (mergedMap, toMap, skipAttributes) {
    ...
    MRS.convertNumericToRSAccountFormat = function (account) {
		return converters.convertNumericToRSAccountFormat(account);
	};

    MRS.getAccountLink = function (object, accountKey, accountRef, title, showAccountRS, clazz) {
        var accountRS;
        if (typeof object[accountKey + "RS"] != "undefined") {
            accountRS = object[accountKey + "RS"];
        } else if (typeof object[accountKey] != "undefined") {
            accountRS = MRS.convertNumericToRSAccountFormat(object[accountKey]);
        } else {
            return '/';
        }
        var accountTitle;
        if (accountRef && accountRS == accountRef) {
            ...
            accountTitle = MRS.getAccountTitle(object, accountKey);
        }
        if (!clazz) {
            clazz = "";
        } else {
    ...
    MRS.getTransactionLink = function(id, text, isEscapedText) {
        if (!text) {
            text = id;
        }
    ...
    MRS.getBlockLink = function(height, text, isEscapedText) {
        if (!text) {
            text = height;
        }
    ...
    MRS.getPeerLink = function(address) {
        if (!address ) {
    ...
    MRS.setBackLink = function() {
        var backLink = $(".back-link");
        if (MRS.modalStack.length > 0) {
            var backModalInfo = MRS.modalStack[MRS.modalStack.length - 1];
    ...
    MRS.getAccountTitle = function (object, acc) {
    ...
    MRS.getAccountFormatted = function (object, acc) {
		var type = typeof object;
    ...
    MRS.dataLoaded = function (data, noPageLoad) {
		var $el = $("#" + MRS.currentPage + "_contents");

		if ($el.length) {
			$el.empty().append(data);
		} else {
			$el = $("#" + MRS.currentPage + "_table");
			$el.find("tbody").empty().append(data);
            $el.find('[data-toggle="tooltip"]').tooltip();
		}

		MRS.dataLoadFinished($el);
    ...
    MRS.dataLoadFinished = function ($el, fadeIn) {
    ...
    MRS.createInfoTable = function (data, fixed) {
		var rows = "";
		for (var key in data) {
			...
			var match = key.match(/(.*)(MQT|QNT|RS)$/);
			var type = "";
            ...
            if (/_formatted_html$/i.test(key)) {
                ...
                if ($.isArray(value)) {
                    value = MRS.formatQuantity(value[0], value[1]);
                } else {
                    value = MRS.formatQuantity(value, 0);
                }
            } else if (key == "price" || key == "total" || key == "amount" || key == "fee" || key == "refund" || key == "discount") {
                value = MRS.formatAmount(new BigInteger(String(value))) + " " + MRS.constants.COIN_SYMBOL;
            } else if (key == "sender" || key == "recipient" || key == "account" || key == "seller" || key == "buyer" || key == "lessee") {
                value = "<a href='#' data-user='" + MRS.escapeRespStr(value) + "' class='show_account_modal_action'>" + MRS.getAccountTitle(value) + "</a>";
            } else if (key == "request_processing_time") { /* Skip from displaying request processing time */
                continue;
            } else {
                value = MRS.escapeRespStr(value).nl2br();
            }
    ...
    MRS.formatStyledAmount = function (strAmount, round) {
        var locale = MRS.getLocale();
        var amount = MRS.formatAmount(strAmount, round).split(locale.decimal);
		if (amount.length == 2) {
    ...
    MRS.getUnconfirmedTransactionsFromCache = function (type, subtype, fields, single) {
		if (!MRS.unconfirmedTransactions.length) {
			return false;
		}
		...
		for (var i = 0; i < MRS.unconfirmedTransactions.length; i++) {
			var unconfirmedTransaction = MRS.unconfirmedTransactions[i];
    ...
    MRS.hasTransactionUpdates = function (transactions) {
		return ((transactions && transactions.length) || MRS.unconfirmedTransactionsChange);
	};

    MRS.showMore = function ($el) {
		if (!$el) {
			$el = $("#" + MRS.currentPage + "_contents");
			if (!$el.length) {
				$el = $("#" + MRS.currentPage + "_table");
			}
		}
    ...
    MRS.showFullDescription = function ($el) {
		$el.addClass("open").removeClass("closed");
		$el.find(".description_toggle").text("Less...");
	};

    MRS.showPartialDescription = function ($el) {
    ...
    $("body").on(".description_toggle", "click", function (e) {
		...
		if ($(this).closest(".description").hasClass("open")) {
			MRS.showPartialDescription();
		} else {
			MRS.showFullDescription();
		}
	});
    ...
    MRS.translateServerError = function (response) {
        var match;
			...
			case 3:
				...
				if (match && match[1]) {
					return $.t("error_not_specified", {
						"name": MRS.getTranslatedFieldName(match[1]).toLowerCase()
					}).capitalize();
				}
                ...
                if (match && match[1]) {
                    ...
                    for (var i=0; i<fieldNames.length; i++) {
                        translatedFieldNames.push(MRS.getTranslatedFieldName(fieldNames[i].toLowerCase()));
                    }
			...
			case 8:
				switch (response.errorDescription) {
					...
					case "recipient account does not have public key":
						return $.t("error_recipient_no_public_key", { "symbol": MRS.constants.COIN_SYMBOL });
					default:
						return response.errorDescription;
						break;
				}
    ...
    MRS.getTranslatedFieldName = function (name) {
        var nameKey = String(name).replace(/MQT|QNT|RS$/, "").replace(/\s+/g, "").replace(/([A-Z])/g, function ($1) {
			return "_" + $1.toLowerCase();
		});
    ...
    MRS.isControlKey = function (charCode) {
        return !(charCode >= 32 || charCode == 10 || charCode == 13);
    };

    MRS.validateDecimals = function (maxFractionLength, charCode, val, e) {
        ...
        if (mantissa && mantissa[1].length > maxFractionLength) {
            var selectedText = MRS.getSelectedText();
    ...
    MRS.getTransactionStatusIcon = function (phasedEntity) {
        var statusIcon;
    ...
    MRS.getAccountForDecryption = function(transaction, recipient, sender) {
        if (!recipient && transaction.recipient == MRS.account) {
            return transaction.sender;
        }
        if (transaction[recipient] == MRS.account) {
            return transaction.sender || transaction[sender];
        }
        if (!sender && transaction.sender == MRS.account) {
    ...
    MRS.phasingControlObjectToPhasingParams = function(controlObj) {
        var phasingParams = {};
    ...
    MRS.addEllipsis = function(str, length) {
    ...
    MRS.generateToken = function(message, secretPhrase) {
        var messageBytes = MRS.getUtf8Bytes(message);
        var pubKeyBytes = converters.hexStringToByteArray(MRS.getPublicKey(converters.stringToHexString(secretPhrase)));
        var token = pubKeyBytes;

        var tsb = [];
        var ts = MRS.toEpochTime();
        tsb[0] = ts & 0xFF;
        tsb[1] = (ts >> 8) & 0xFF;
        tsb[2] = (ts >> 16) & 0xFF;
        tsb[3] = (ts >> 24) & 0xFF;
        ...
        token = token.concat(tsb, converters.hexStringToByteArray(
            MRS.signBytes(converters.byteArrayToHexString(messageBytes),
    ...
    MRS.versionCompare = function (v1, v2) {
        if (v2 == undefined) {
            return -1;
     ...
     * @param obj
     */
    MRS.escapeResponseObjStrings = function (obj) {
        for (var key in obj) {
            if (obj.hasOwnProperty(key)) {
                ...
                if (typeof val === 'string') {
                    ...
                    MRS.escapeResponseObjStrings(obj[key]);
                }
            }
        }
    };
     ...
     * @param val
     */
    MRS.escapeRespStr = function (val) {
        return String(val).unescapeHTML().escapeHTML();
    };

    MRS.unescapeRespStr = function (val) {
        return String(val).unescapeHTML();
    };

    MRS.getRandomPermutation = function(array) {
    ...
    MRS.getErrorMessage = function(response) {
    ...
    MRS.isRsAccount = function(account) {
        return MRS.isRsAccountImpl(account, MRS.constants.ACCOUNT_RS_MATCH ? MRS.constants.ACCOUNT_RS_MATCH : MRS.getRsAccountRegex("MTR"));
    };

    MRS.isRsAccountImpl = function(account, regex) {
        return regex.test(account);
    };

    MRS.isNumericAccount = function(account) {
        return MRS.isNumericAccountImpl(account, MRS.constants.ACCOUNT_NUMERIC_MATCH);
    };

    MRS.isNumericAccountImpl = function(account, regex) {
        return regex.test(account);
    };

    MRS.getAccountMask = function(c) {
        switch(c) {
            case "*":
                return MRS.constants.ACCOUNT_MASK_ASTERIX;
            case "_":
                return MRS.constants.ACCOUNT_MASK_UNDERSCORE;
            default:
                return MRS.constants.ACCOUNT_MASK_PREFIX ? MRS.constants.ACCOUNT_MASK_PREFIX : "MTR-";
        }
    };

    MRS.printPaperWallet = function(passphrase) {
        var $pageHeader = $("<h2 data-i18n='metro_paper_wallet'>Metro Paper Wallet</h2>");
        var $passphraseHeader = $("<h3 data-i18n='passphrase'>Passphrase</h3>");
        var $passphraseText = $("<div></div>");
        $passphraseText.text(passphrase);
        var passphraseImg = MRS.generateQRCode(null, passphrase, 2, 4);
        $(passphraseImg).load(function() {
            var account = MRS.getAccountId(passphrase, true);
            var $accountHeader = $("<h3 data-i18n='account'>Account</h3>");
            var $accountText = $("<div></div>");
            $accountText.html(account);
            var accountImg = MRS.generateQRCode(null, account, 2, 4);
