...
public final class Shuffling {
    ...
    private static final boolean deleteFinished = Metro.getBooleanProperty("metro.deleteFinishedShufflings");
        ...
        try (DbIterator<ShufflingParticipant> participants = ShufflingParticipant.getParticipants(id)) {
            for (ShufflingParticipant participant : participants) {
                ...
                if (holdingType != HoldingType.MTR) {
                    participantAccount.addToBalanceMQT(event, this.id, -Constants.SHUFFLING_DEPOSIT_MQT);
                }
            }
        }
        for (byte[] recipientPublicKey : recipientPublicKeys) {
            ...
            if (holdingType != HoldingType.MTR) {
                recipientAccount.addToBalanceAndUnconfirmedBalanceMQT(event, this.id, Constants.SHUFFLING_DEPOSIT_MQT);
            }
        }
