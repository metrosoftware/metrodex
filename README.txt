...
jar/exe/dmg package, or run "java -jar metro-client.jar" to start the
...
desktop icons to start the Metro server.


Using the metro-client.zip package:

Unpack the metro-client.zip package and open a shell in the resulting metro
...
the metro_db blockchain database. The start.sh script can be run from any
directory, it starts the java process in the background and uses ~/.metro for
configuration files, logs, and the metro_db database. Unlike run.sh, start.sh
...
message "Metro server 1.x.x started successfully" in the console log. If run in
desktop mode, a JavaFX window will open automatically. Otherwise, open a
browser, without stopping the java process, and go to http://localhost:7886 ,
where the Metro UI should now be available.
...
to the Metro installation directory, as the use of special characters may result
...
conf/metro-default.properties file. All possible settings are listed, with
detailed explanation. If you decide to change any setting, do not edit
metro-default.properties directly, but create a new conf/metro.properties file
and only add to it the properties that need to be different from the default
values. You do not need to delete the defaults from metro-default.properties, the
settings in metro.properties override those in metro-default.properties. This way,
when upgrading the software, you can safely overwrite metro-default.properties
with the updated file from the new package, while your customizations remain
safe in the metro.properties file.


How to contribute?

There are many ways to contribute to Metro. Here are some examples:
...
The Metro software is a client-server application. It consists of a java server
...
The java process communicates with peers on port 57874 tcp by default. If you are
...
requests which other Metro client applications could use.

The blockchain is stored on disk using the H2 embedded database, inside the
metro_db directory. When upgrading, you should not delete the old metro_db
...
metro_db, except that it will take some extra time to download the blockchain
from scratch.

The default Metro client does not store any wallet-type file on disk. Unlike
...
which you see in the console window, but also to a file metro.log, which gets
overwritten at restart. In case of an error, the metro.log file may contain
...
http://localhost:7886/doc - a javadoc documentation for client developers who
...
metro.jar file using the enclosed jar.sh script, and then metro.jar should be
