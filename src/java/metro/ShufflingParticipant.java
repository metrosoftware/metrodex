...
import metro.db.PrunableDbTable;
...
public final class ShufflingParticipant {
    ...
    void setData(byte[][] data, long timestamp) {
        if (data != null && Metro.getEpochTime() - timestamp < Constants.MAX_PRUNABLE_LIFETIME && getData() == null) {
            shufflingDataTable.insert(new ShufflingData(shufflingId, accountId, data, timestamp, Metro.getBlockchain().getHeight()));
        }
    }
