...
/**
 * Compact and reorganize the MRS database.  The MRS application must not be
 * running.
 *
 * To run the database compact tool on Linux or Mac:
 *
 *   java -cp "classes:lib/*:conf" metro.tools.CompactDatabase
 *
 * To run the database compact tool on Windows:
 *
 *   java -cp "classes;lib/*;conf" -Dmetro.runtime.mode=desktop metro.tools.CompactDatabase
 */
public class CompactDatabase {

    /**
     * Compact the MRS database
     *
     * @param   args                Command line arguments
     */
    public static void main(String[] args) {
        //
        // Initialize Metro properties and logging
        //
        Logger.init();
        //
    ...
    private static int compactDatabase() {
        ...
        //
        String dbPrefix = Constants.isTestnet ? "metro.testDb" : "metro.db";
        String dbType = Metro.getStringProperty(dbPrefix + "Type");
        if (!"h2".equals(dbType)) {
            Logger.logErrorMessage("Database type must be 'h2'");
            return 1;
        }
        String dbUrl = Metro.getStringProperty(dbPrefix + "Url");
        if (dbUrl == null) {
            String dbPath = Metro.getDbDir(Metro.getStringProperty(dbPrefix + "Dir"));
            dbUrl = String.format("jdbc:%s:%s", dbType, dbPath);
        }
        String dbParams = Metro.getStringProperty(dbPrefix + "Params");
        dbUrl += ";" + dbParams;
        if (!dbUrl.contains("MV_STORE=")) {
            dbUrl += ";MV_STORE=FALSE";
        }
        String dbUsername = Metro.getStringProperty(dbPrefix + "Username", "sa");
        String dbPassword = Metro.getStringProperty(dbPrefix + "Password", "sa", true);
        //
        ...
        File dbFile = new File(dbDir, "metro.h2.db");
        if (!dbFile.exists()) {
            dbFile = new File(dbDir, "metro.mv.db");
            if (!dbFile.exists()) {
                Logger.logErrorMessage("MRS database not found");
                return 1;
            }
        }
        ...
        try {
            ...
            switch (phase) {
                ...
                case 1:
                    ...
                    //
                    File newFile = new File(dbDir, "metro.h2.db");
                    if (newFile.exists()) {
                        if (!newFile.delete()) {
                            Logger.logErrorMessage(String.format("Unable to delete '%s'", newFile.getPath()));
                        }
                    } else {
                        newFile = new File(dbDir, "metro.mv.db");
                        if (newFile.exists()) {
                            if (!newFile.delete()) {
                                Logger.logErrorMessage(String.format("Unable to delete '%s'", newFile.getPath()));
                            }
