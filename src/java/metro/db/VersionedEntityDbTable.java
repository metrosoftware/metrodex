...
public abstract class VersionedEntityDbTable<T> extends EntityDbTable<T> {
    ...
    public final boolean delete(T t, boolean keepInCache) {
        ...
        try (Connection con = db.getConnection();
            ...
            pstmtCount.setInt(i, Metro.getBlockchain().getHeight());
    ...
    public static void trim(final TransactionalDb db, final String table, final int height, final DbKey.Factory dbKeyFactory, int trimStartHeight) {
        if (!db.isInTransaction()) {
            throw new IllegalStateException("Not in transaction");
        }
        try (Connection con = db.getConnection();
             ...
             PreparedStatement pstmtDelete = con.prepareStatement("DELETE FROM " + table + dbKeyFactory.getPKClause()
                     + " AND height < ? AND height >= " + trimStartHeight + " LIMIT " + Constants.BATCH_COMMIT_SIZE);
