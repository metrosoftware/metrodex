...
var MRS = (function(MRS, $) {
    ...
    hallmarkModal.on("show.bs.modal", function() {
        ...
        if (MRS.upnpExternalAddress) {
            $("#generate_hallmark_host").val(MRS.upnpExternalAddress);
        }
		$("#hallmark_modal_button").text($.t("generate")).data("form", "generate_hallmark_form");
	});

	MRS.forms.markHost = function() {
        return {};
	};

	MRS.forms.markHostComplete = function(response) {
        var hallmarkModal = $("#hallmark_modal");
        hallmarkModal.find(".error_message").hide();

		if (response.hallmark) {
			$("#generate_hallmark_output").html($.t("generated_hallmark_is") + "<br /><br /><textarea readonly style='width:100%' rows='4'>" + MRS.escapeRespStr(response.hallmark) + "</textarea>").show();
		} else {
	...
	MRS.forms.markHostError = function() {
		$("#generate_hallmark_output").hide();
	};

	MRS.forms.decodeHallmarkComplete = function(response) {
		...
		if (!response.errorCode) {
            ...
            $("#parse_hallmark_account").val(MRS.convertNumericToRSAccountFormat(response.account));
			...
			$("#parse_hallmark_output").html($.t("error_invalid_hallmark", {
				"error": MRS.escapeRespStr(response.errorDescription)
	...
	MRS.forms.decodeHallmarkError = function() {
